...
description: Descubra la clase Aspose.Words.ImageSavingArgs, que mejora el procesamiento de documentos con opciones de guardado de imágenes personalizables para un rendimiento óptimo.
type: docs
weight: 5990
...
| [IsImageAvailable](../../aspose.words.saving/imagesavingargs/isimageavailable/) { get; } | Devuelve`verdadero` Si la imagen actual está disponible para exportar. |
...
De forma predeterminada, cuando Aspose.Words guarda un documento como HTML, guarda cada imagen en un archivo separado. Aspose.Words utiliza el nombre del archivo del documento y un número único para generar un nombre de archivo único para cada imagen del documento.

`ImageSavingArgs`permite redefinir cómo se generan los nombres de los archivos de imagen o evitar por completo el guardado de imágenes en archivos proporcionando sus propios objetos de flujo.

Para aplicar su propia lógica para generar nombres de archivos de imagen, utilice [`ImageFileName`](./imagefilename/) ,[`CurrentShape`](./currentshape/) y[`IsImageAvailable`](./isimageavailable/) propiedades.
