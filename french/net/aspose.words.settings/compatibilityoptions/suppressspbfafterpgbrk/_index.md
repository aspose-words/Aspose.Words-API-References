---
title: CompatibilityOptions.SuppressSpBfAfterPgBrk
second_title: Référence de l'API Aspose.Words pour .NET
description: CompatibilityOptions propriété. Ne pas utiliser despace avant sur la première ligne après un saut de page.
type: docs
weight: 500
url: /fr/net/aspose.words.settings/compatibilityoptions/suppressspbfafterpgbrk/
---
## CompatibilityOptions.SuppressSpBfAfterPgBrk property

Ne pas utiliser d'espace avant sur la première ligne après un saut de page.

```csharp
public bool SuppressSpBfAfterPgBrk { get; set; }
```

### Voir également

* class [CompatibilityOptions](../)
* espace de noms [Aspose.Words.Settings](../../compatibilityoptions/)
* Assemblée [Aspose.Words](../../../)


