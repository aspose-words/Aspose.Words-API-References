...
description: Aspose.Words.Settings.OdsoRecipientData classe. Représente des informations sur un seul enregistrement dans une source de données externe qui doit être exclue du publipostage.
type: docs
weight: 5630
...
| [Hash](../../aspose.words.settings/odsorecipientdata/hash/) { get; set; } | Représente le code de hachage de cet enregistrement. Microsoft Word utilise parfois[`Hash`](./hash/) d'un enregistrement entier au lieu d'un[`UniqueTag`](./uniquetag/) valeur. La valeur par défaut est 0. |
...
Si un enregistrement doit être fusionné dans un document fusionné, aucune information n'est nécessaire sur cet enregistrement. Toutefois, si un enregistrement donné ne doit pas être fusionné dans un document fusionné, la valeur de la clé unique pour cet enregistrement doit être stockée dans le[`UniqueTag`](./uniquetag/) propriété de cet objet pour indiquer cette exclusion.
