...
description: Document méthode. Rend une page de document dans unGraphics objet à une taille spécifiée.
type: docs
weight: 670
...
| width | Single | La largeur maximale (en unités universelles) pouvant être occupée par la page rendue. |
| height | Single | La hauteur maximale (en unités universelles) pouvant être occupée par la page rendue. |

### Return_Value

L'échelle qui a été automatiquement calculée pour que la page rendue corresponde à la taille spécifiée.

### Exemples

Montre comment rendre le document sous forme de bitmap à un emplacement et à une taille spécifiés (.NetStandard 2.0).
...
using (SKBitmap bitmap = new SKBitmap(700, 700))
{
    using (SKCanvas canvas = new SKCanvas(bitmap))
    {
        ...
        // Décalez la page de 0,5" des bords supérieur et gauche de la page.
        canvas.Translate(0.5f, 0.5f);

        // Fait pivoter la page rendue de 10 degrés.
        ...
        // Affiche la première page du document à la même taille que le rectangle ci-dessus.
        ...
        // Affiche à nouveau la première page dans le rectangle nouvellement créé.
...
Montre comment restituer un document dans un bitmap à un emplacement et à une taille spécifiés.
...
using (Bitmap bmp = new Bitmap(700, 700))
{
    using (Graphics gr = Graphics.FromImage(bmp))
    {
        ...
        // Décalez la sortie de 0,5" du bord.
        gr.TranslateTransform(0.5f, 0.5f);

        // Fait pivoter la sortie de 10 degrés.
        gr.RotateTransform(10);

        // Dessine un rectangle de 3"x3".
        ...
        gr.ResetTransform();
         ...
         // Appliquez un autre ensemble de transformations.
        gr.TranslateTransform(10, 10);
        gr.ScaleTransform(0.5f, 0.5f);
        gr.PageScale = 2f;

        // Crée un autre rectangle et l'utilise pour encadrer une autre page du document.
