---
title: Position
second_title: Référence de l'API Aspose.Words pour .NET
description: Obtient ou définit la position du texte en points par rapport à la ligne de base. Un nombre positif élève le texte et un nombre négatif labaisse.
type: docs
weight: 300
url: /fr/net/aspose.words/font/position/
---
## Font.Position property

Obtient ou définit la position du texte (en points) par rapport à la ligne de base. Un nombre positif élève le texte et un nombre négatif l'abaisse.

```csharp
public double Position { get; set; }
```

### Exemples

Montre comment formater du texte pour décaler sa position.

```csharp
Document doc = new Document();
Paragraph para = (Paragraph) doc.GetChild(NodeType.Paragraph, 0, true);

// Élève cette suite de texte de 5 points au-dessus de la ligne de base.
Run run = new Run(doc, "Raised text. ");
run.Font.Position = 5;
para.AppendChild(run);

// Abaissez cette série de texte de 10 points en dessous de la ligne de base.
run = new Run(doc, "Lowered text. ");
run.Font.Position = -10;
para.AppendChild(run);

// Ajoute une suite de texte normal.
run = new Run(doc, "Text in its default position. ");
para.AppendChild(run);

// Ajoute une suite de texte qui apparaît en indice.
run = new Run(doc, "Subscript. ");
run.Font.Subscript = true;
para.AppendChild(run);

// Ajoute une suite de texte qui apparaît en exposant.
run = new Run(doc, "Superscript.");
run.Font.Superscript = true;
para.AppendChild(run);

doc.Save(ArtifactsDir + "Font.PositionSubscript.docx");
```

### Voir également

* class [Font](../)
* espace de noms [Aspose.Words](../../font/)
* Assemblée [Aspose.Words](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Words.dll -->
