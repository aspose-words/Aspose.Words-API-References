...
description: Le moteur de publipostage Aspose.Words appelle cette méthode lorsquil rencontre le début dune région de publipostage imbriquée.
type: docs
weight: 20
...
Si la table représentée par cet objet de source de données a une table enfant (détail) associée avec le nom spécifié, , votre implémentation doit alors renvoyer un nouveau[`IMailMergeDataSource`](../../imailmergedatasource) objet qui fournira access aux enregistrements enfants de l'enregistrement actuel. Un exemple de ceci est la relation Orders / OrderDetails. Supposons que le courant[`IMailMergeDataSource`](../../imailmergedatasource) object représente la table Commandes et possède un enregistrement de commande en cours. Ensuite, Aspose.Words rencontre "MERGEFIELD TableStart:OrderDetails" dans le document et appelle`GetChildDataSource` . Vous devez créer et retourner un[`IMailMergeDataSource`](../../imailmergedatasource) objet qui permettra à Aspose.Words d'accéder à l'enregistrement OrderDetails pour la commande en cours.

Si cet objet de source de données n'a pas de relation avec la table avec le nom spécifié, alors vous devez return a[`IMailMergeDataSource`](../../imailmergedatasource)objet qui donnera accès à tous les enregistrements de la table spécifiée.
