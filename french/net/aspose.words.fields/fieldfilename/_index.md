...
description: Découvrez la classe Aspose.Words.Fields.FieldFileName, conçue pour simplifier l'implémentation du champ FILENAME pour une automatisation transparente des documents.
type: docs
weight: 2280
...
| [IncludeFullPath](../../aspose.words.fields/fieldfilename/includefullpath/) { get; set; } | Obtient ou définit s'il faut inclure le nom du chemin d'accès complet au fichier. |
...
Dans l'implémentation actuelle, utilise le[`OriginalFileName`](../../aspose.words/document/originalfilename/) Propriété permettant de récupérer le nom du fichier. Si le document a été chargé depuis un flux ou créé vierge, le nom du fichier d'enregistrement est utilisé (s'il est connu).
