...
| [IsValid](../../aspose.words.digitalsignatures/digitalsignaturecollection/isvalid/) { get; } | Retours`vrai` si toutes les signatures numériques de cette collection sont valides et que le document n'a pas été falsifié, with Renvoie également`vrai` s'il n'y a pas de signatures numériques. `FAUX` si au moins une signature numérique est invalide. |
