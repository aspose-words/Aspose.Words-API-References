...
description: Découvrez la propriété DataSources de ReportBuilderContext pour une création de rapports transparente avec des sources de données polyvalentes adaptées à vos besoins.
type: docs
weight: 20
url: /fr/net/aspose.words.lowcode/reportbuildercontext/datasources/
---
...
La clé représente la source de données, tandis que la valeur est le nom de la source de données. Le nom de la source de données peut être nul ou une chaîne vide ; dans de tels cas, le moteur de reporting détectera automatiquement le nom de la source de données à partir de la source de données spécifiée.
