...
description: Découvrez la propriété ImagesFolderAlias de XamlFlowSaveOptions pour personnaliser les chemins d'URI des images dans les documents XAML. Optimisez vos projets en toute simplicité !
type: docs
weight: 40
...
Lorsque vous enregistrez un[`Document`](../../../aspose.words/document/) au format XAML, Aspose.Words doit enregistrer toutes les images intégrées dans le document en tant que fichiers autonomes.[`ImagesFolder`](../imagesfolder/) vous permet de spécifier où les images seront enregistrées et`ImagesFolderAlias` permet de spécifier comment les URI des images seront construits.
...
Si`ImagesFolderAlias` est défini sur '.' (point), alors le nom du fichier image sera écrit en XAML sans chemin, quelles que soient les autres options.
