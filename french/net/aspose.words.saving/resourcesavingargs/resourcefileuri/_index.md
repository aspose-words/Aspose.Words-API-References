...
description: ResourceSavingArgs propriété. Obtient ou définit lURI Uniform Resource Identifier utilisé pour référencer le fichier de ressources à partir du document.
type: docs
weight: 40
...
Obtient ou définit l'URI (Uniform Resource Identifier) utilisé pour référencer le fichier de ressources à partir du document.
...
Aspose.Words génère automatiquement un URI pour chaque fichier de ressources lors de l'exportation au format HTML ou SVG à page fixe. Les fichiers de ressources de référence URI générés enregistrés par Aspose.Words. Cependant, les URI peuvent être incorrects si les fichiers de ressources doivent être déplacés vers un autre emplacement ou si les fichiers de ressources sont enregistrés dans des flux. Cette propriété permet de corriger les URI dans ces cas.

Lorsque l'événement est déclenché, cette propriété contient l'URI qui a été généré par Aspose.Words. Vous pouvez modifier la valeur de cette propriété pour fournir un URI personnalisé pour le fichier de ressources.
