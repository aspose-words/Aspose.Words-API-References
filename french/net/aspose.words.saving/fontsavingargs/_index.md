...
description: Découvrez la classe Aspose.Words.Saving.FontSavingArgs  améliorez le traitement des documents avec des données d'événement FontSaving détaillées pour une personnalisation et un contrôle supérieurs.
type: docs
weight: 5780
...
| [IsSubsettingNeeded](../../aspose.words.saving/fontsavingargs/issubsettingneeded/) { get; set; } | Permet de spécifier si la police actuelle sera sous-ensemble avant d'être exportée en tant que ressource de police. |
| [Italic](../../aspose.words.saving/fontsavingargs/italic/) { get; } | Indique si la police actuelle est en italique. |
| [KeepFontStreamOpen](../../aspose.words.saving/fontsavingargs/keepfontstreamopen/) { get; set; } | Spécifie si Aspose.Words doit garder le flux ouvert ou le fermer après l'enregistrement d'une police. |
...
`FontSavingArgs`permet également de redéfinir la manière dont les noms de fichiers de polices sont générés ou de contourner complètement l'enregistrement des polices dans des fichiers en fournissant vos propres objets de flux.

Pour décider d'enregistrer ou non une ressource de police particulière, utilisez le[`IsExportNeeded`](./isexportneeded/) propriété.

Pour enregistrer les polices dans des flux plutôt que dans des fichiers, utilisez le[`FontStream`](./fontstream/) propriété.
