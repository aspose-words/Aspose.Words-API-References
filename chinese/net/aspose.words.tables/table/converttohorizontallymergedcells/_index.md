...
description: 将按宽度水平合并的单元格转换为按HorizontalMergeaspose.words.tables/cellformat/horizontalmerge合并的单元格
type: docs
weight: 390
...
将按宽度水平合并的单元格转换为按[`HorizontalMerge`](../../cellformat/horizontalmerge)合并的单元格。
...
表格单元格可以使用合并标志HorizontalMerge或使用单元格宽度[`Width`](../../cellformat/width)。

当表格单元格通过宽度属性合并时[`HorizontalMerge`](../../cellformat/horizontalmerge)没有意义，但有时有合并标志是比较方便的方式。

使用此方法将按宽度水平合并的表格单元格转换为按合并标志合并的单元格。

### 例子

显示如何将按宽度水平合并的单元格转换为由 CellFormat.HorizontalMerge 合并的单元格。

```csharp
Document doc = new Document(MyDir + "Table with merged cells.docx");

 // Microsoft Word 不再编写合并标志，而是按宽度定义合并单元格。
 // Aspose.Words默认只定义一行5个单元格，并且没有一个有水平合并标志，
...
Assert.True(row.Cells.All(c => ((Cell)c).CellFormat.HorizontalMerge == CellMerge.None));
 ...
 // 使用“ConvertToHorizontallyMergedCells”方法水平转换单元格 merge
 // 按其宽度到由 flags.
 水平合并的单元格
