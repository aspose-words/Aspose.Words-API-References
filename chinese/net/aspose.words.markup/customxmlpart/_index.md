...
description: 探索 Aspose.Words.Markup.CustomXmlPart 类，高效管理包内自定义 XML 数据存储。立即增强您的文档处理能力！
type: docs
weight: 4610
...
| [DataChecksum](../../aspose.words.markup/customxmlpart/datachecksum/) { get; } | 指定循环冗余校验 (CRC)[`Data`](./data/)内容. |
...
| 姓名 | 描述 |
| --- | --- |
| [Clone](../../aspose.words.markup/customxmlpart/clone/)() | 制作对象的“足够深”副本。 不复制[`Data`](./data/)值. |

## 评论

DOCX 或 DOC 文档可以包含一个或多个自定义 XML 数据存储部分。Aspose.Words 保存自定义 XML 数据，并通过 允许创建和提取自定义 XML 数据。[`CustomXmlParts`](../../aspose.words/document/customxmlparts/)收藏。
