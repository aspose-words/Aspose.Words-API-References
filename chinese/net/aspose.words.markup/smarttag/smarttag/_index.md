...
description: 初始化SmartTagaspose.words.markup/smarttag类的新实例
type: docs
weight: 10
...
创建新节点时，需要指定该节点所属的文档。 一个节点不能没有文档存在，因为它依赖于文档范围的结构 例如列表和样式。尽管一个节点总是属于一个文档，但一个节点可能或可能 不是文档树的一部分。

当一个节点被创建时，它属于一个文档，但还不是文档树的一部分 和[`ParentNode`](../../../aspose.words/node/parentnode)为空。要将节点插入到文档中，请使用 [`InsertAfter`](../../../aspose.words/compositenode/insertafter)或[`InsertBefore`](../../../aspose.words/compositenode/insertbefore)方法 在父节点上。
