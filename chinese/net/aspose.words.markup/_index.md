...
description: 这Aspose.Words.Markup命名空间包含 表示文档中客户定义的语义的类智能标记自定义 XML 和 结构化文档标记内容控件 在 C#.
type: docs
weight: 160
url: /zh/net/aspose.words.markup/
---
这**Aspose.Words.Markup**命名空间包含 表示文档中客户定义的语义的类：智能标记、自定义 XML 和 结构化文档标记（内容控件）。

## 课程

| 班级 | 描述 |
| --- | --- |
| [CustomPart](./custompart/) | 表示自定义（任意内容）部分，ISO/IEC 29500 标准未定义。 |
| [CustomPartCollection](./custompartcollection/) | 代表集合[`CustomPart`](../aspose.words.markup/custompart/)对象. |
...
| [SdtListItem](./sdtlistitem/) | 该元素指定父级中的单个列表项ComboBox或者DropDownList结构化文档标签. |
...
| [StructuredDocumentTagRangeStart](./structureddocumenttagrangestart/) | 代表开始**远程**接受多部分内容的结构化文档标签。 另请参阅[`StructuredDocumentTagRangeEnd`](../aspose.words.markup/structureddocumenttagrangeend/). |
| [XmlMapping](./xmlmapping/) | 指定用于在parent 结构化文档标记与存储在文档中的自定义XML 数据部分中的XML 元素之间建立映射的信息。 |
## 接口
...
| 枚举 | 描述 |
| --- | --- |
| [MarkupLevel](./markuplevel/) | 指定文档树中特定的级别[`StructuredDocumentTag`](../aspose.words.markup/structureddocumenttag/)可能会发生. |
| [SdtAppearance](./sdtappearance/) | 指定结构化文档标记的外观。 |
...
| [SdtType](./sdttype/) | 指定结构化文档标记 (SDT) 节点的类型。 |
