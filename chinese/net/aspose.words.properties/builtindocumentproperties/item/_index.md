...
description: 按属性名称返回DocumentPropertyaspose.words.properties/documentproperty对象
type: docs
weight: 130
...
属性的字符串名称对应于键入的名称 可从[`BuiltInDocumentProperties`](../../builtindocumentproperties)获得的属性。

如果您请求的属性不在文档中，但该属性的名称 被识别为有效的内置名称，一个新的[`DocumentProperty`](../../documentproperty) 被创建，添加到集合并返回。新创建的属性被赋予 一个默认值（空字符串、零、false 或 DateTime.MinValue，具体取决于内置属性的 类型）。
