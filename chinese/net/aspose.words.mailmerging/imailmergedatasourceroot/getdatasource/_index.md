...
description: Aspose.Words 邮件合并引擎在遇到顶级邮件合并区域的开头时调用此方法
type: docs
weight: 10
...
当 Aspose.Words 邮件合并引擎使用数据并遇到 MERGEFIELD TableStart:TableName, 它在此对象上调用`GetDataSource`。您的实现需要返回一个新的数据源对象。 Aspose.Words 将使用返回的数据源来填充邮件合并区域。

如果具有指定名称的数据源（表）不存在，您的实现应该返回` null` 。
...
/// <summary>
 /// 可以直接传递到邮件合并中的数据源根，可以注册并包含许多子数据源。
/// 这些源都必须实现IMailMergeDataSource，并且通过一个name
注册和区分
/// 对应于将读取相应数据的邮件合并区域。
/// </summary>
