...
使用此方法在文档中使用来自 的值填充邮件合并字段任何数据源，例如列表或哈希表或对象。您需要编写自己的 类来实现[`IMailMergeDataSource`](../../imailmergedatasource)接口。
