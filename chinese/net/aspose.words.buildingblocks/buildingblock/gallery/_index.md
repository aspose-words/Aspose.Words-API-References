...
description: 为 分类或用户界面排序指定构建块的第一级分类
type: docs
weight: 60
...
Microsoft Word 用户界面中的构建块排列 到画廊中。每个`Gallery`可以有多个类别。 a[`Category`](../category)中的每个块都有一个[`Name`](../name).

对应于 OOXML 中的 **docPartPr.category.gallery** 元素。
