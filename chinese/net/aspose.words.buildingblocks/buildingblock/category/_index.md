...
Microsoft Word 用户界面中的构建块排列 到画廊中。每个[`Gallery`](../gallery)可以有多个类别。 a`Category`中的每个块都有一个[`Name`](../name).

不能为` null` 且不能为空字符串。

对应于 OOXML 中的 **docPartPr.category.name** 元素。
