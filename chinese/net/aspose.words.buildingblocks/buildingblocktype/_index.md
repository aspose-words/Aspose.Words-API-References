...
description: 指定构建块类型该类型可能会影响构建基块 在 Microsoft Word 中的可见性和行为
type: docs
weight: 160
...
指定构建块类型。该类型可能会影响构建基块 在 Microsoft Word 中的可见性和行为。
...
| --- | --- | --- |
| None | `0` | 没有为构建块指定类型信息。 |
| AutomaticallyReplaceNameWithContent | `1` | 允许在将 名称输入应用程序时自动将构建块插入到文档中。 |
| StructuredDocumentTagPlaceholderText | `2` | 构建块是结构化的文档标签占位符文本。 |
