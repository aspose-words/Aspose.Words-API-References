...
指定构建块类型。该类型可能会影响 Microsoft Word 中构建块 的可见性和行为。
...
| --- | --- | --- |
| None | `0` | 没有为构建块指定类型信息。 |
| AutomaticallyReplaceNameWithContent | `1` | 每当 将其名称输入应用程序时，就允许构建块自动插入到文档中。 |
| StructuredDocumentTagPlaceholderText | `2` | 构建块是结构化文档标记占位符文本。 |
| FormFieldHelpText | `3` | 构建块是表单字段帮助文本。 |
| Normal | `4` | 构建块是普通（即常规）术语表文档条目。 |
| AutoCorrect | `5` | 构建块与拼写和语法工具相关联。 |
| AutoText | `6` | 构建块是一个自动图文集条目。 |
| All | `7` | 构建块与所有类型相关联。 |
| Default | `0` | 另存为None. |
