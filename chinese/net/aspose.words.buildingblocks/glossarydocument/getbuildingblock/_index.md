...
这是一种方便的方法，可以遍历所有构建块 in此集合并返回匹配 指定画廊、类别和名称的第一个构建块。

Microsoft Word 将构建块组织成画廊。画廊 是使用[`BuildingBlockGallery`](../../buildingblockgallery)枚举预定义的。 在每个画廊中，可以将构建块组织成一个或多个类别。 类别名称是一个字符串。每个构建块都有一个名称。构建块 名称不能保证是唯一的。
