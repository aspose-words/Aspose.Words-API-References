...
description: 探索 Aspose.Words.Fields.FieldSectionPages 类以有效实现 SECTIONPAGES 字段并增强您的文档处理能力。
type: docs
weight: 2780
...
// 从页眉移回主文档并插入两页。
// 所有这些页面都将位于第一部分。我们的字段在每个标题中出现一次，
// 将对本节的当前/总页数进行编号。
builder.MoveToDocumentEnd();
builder.InsertBreak(BreakType.PageBreak);
builder.InsertBreak(BreakType.PageBreak);

// 我们可以使用这样的文档构建器插入一个新部分。
// 这将影响所有即将出现的标题中的 SECTION 和 SECTIONPAGES 字段中显示的值。
builder.InsertBreak(BreakType.SectionBreakNewPage);

// PAGE 字段将继续计算整个文档的页数。
// 我们可以在每个部分手动重置其计数，以逐个部分地跟踪页面。
