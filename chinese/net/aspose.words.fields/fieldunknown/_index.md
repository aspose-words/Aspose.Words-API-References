...
description: 探索 Aspose.Words.Fields.FieldUnknown 类，高效处理无法识别的字段。无缝集成，增强您的文档处理能力！
type: docs
weight: 2960
...
| 姓名 | 描述 |
| --- | --- |
| [FieldUnknown](fieldunknown/)() | 默认构造函数。 |
...
展示如何使用文档中的“FieldNone”字段。

```csharp
Document doc = new Document();
DocumentBuilder builder = new DocumentBuilder(doc);

// 插入一个在字段代码中未表示目标字段类型的字段。
Field field = builder.InsertField(" NOTAREALFIELD //一个”）;

// “FieldNone”字段类型专为此类字段保留。
Assert.AreEqual(FieldType.FieldNone, field.Type);

// 我们仍然可以使用这些字段并将它们分配为 FieldUnknown 类的实例。
