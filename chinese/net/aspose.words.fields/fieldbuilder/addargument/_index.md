...
description: 使用 FieldBuilder 的 AddArgument 方法增强您的编码，该方法旨在轻松添加字段参数以简化开发。
type: docs
weight: 20
...
添加由另一个表示的子字段[`FieldBuilder`](../)到字段的代码。
...
添加由以下表示的字段参数[`FieldArgumentBuilder`](../../fieldargumentbuilder/)到字段的代码。
...
当参数由不同部分（例如子字段、节点和纯文本）混合组成时使用此重载。
