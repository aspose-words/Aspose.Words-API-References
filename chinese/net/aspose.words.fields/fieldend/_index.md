...
| [HasSeparator](../../aspose.words.fields/fieldend/hasseparator) { get; } | 返回 **true** 如果此字段有分隔符。 |
...
| [IsDirty](../../aspose.words.fields/fieldchar/isdirty) { get; set; } | 获取或设置字段的当前结果是否由于对文档的其他修改 而不再正确（陈旧）。 |
...
| 姓名 | 描述 |
| --- | --- |
| override [Accept](../../aspose.words.fields/fieldend/accept)(DocumentVisitor) | 接受访问者。 |
...
[`FieldEnd`](../fieldend)是一个内联级节点，并由文档中的[`FieldEndChar`](../../aspose.words/controlchar/fieldendchar)控制字符表示 。

[`FieldEnd`](../fieldend)只能是Words的子级。段落。

Microsoft Word 文档中的完整字段是由 字段起始字符、字段代码、字段分隔符、字段组成的复杂结构结果 和字段结束字符。有些字段只有字段开始、字段代码和字段结束。

要在文档中轻松插入新字段，请使用[`InsertField`](../../aspose.words/documentbuilder/insertfield) 方法。
