...
| [TextAfter](../../aspose.words.fields/fieldmergefield/textafter) { get; set; } | 如果字段不为空，则获取或设置要在字段后插入的文本。 |
| [TextBefore](../../aspose.words.fields/fieldmergefield/textbefore) { get; set; } | 如果字段不为空，则获取或设置要在字段前插入的文本。 |
...
在邮件合并主文档的合并字符中检索数据字段的名称。 当主文档与所选数据源合并时，将插入来自指定 数据字段的信息以代替合并字段。
