...
什么时候[`TextInputType`](../textinputtype/)是Calculated，那么这个字符串保存 要计算的表达式。该表达式必须是符合 Microsoft Word 公式 field 要求的有效公式。当您使用此属性设置新表达式时，Aspose.Words 会自动计算公式 result 并将其插入表单字段。
