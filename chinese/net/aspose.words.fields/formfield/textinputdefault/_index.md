...
description: 探索 FormField TextInputDefault 属性，轻松为表单设置默认文本或计算，增强用户体验和数据准确性。
type: docs
weight: 190
...
此属性的含义取决于[`TextInputType`](../textinputtype/)财产。

什么时候[`TextInputType`](../textinputtype/)是Regular或 Number，此字符串指定文本表单字段的默认字符串。 此字符串是当表单字段为空时，Microsoft Word 将在文档中显示的内容。

什么时候[`TextInputType`](../textinputtype/)是Calculated，则此字符串 contains 为要计算的表达式。该表达式必须符合 Microsoft Word formula field 的要求。当您使用此属性设置新的表达式时，Aspose.Words 会自动计算公式 result 并将其插入到表单字段中。
