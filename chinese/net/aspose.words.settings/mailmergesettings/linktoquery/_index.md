...
description: 不确定这个 Microsoft Word 自动化参考建议这指定每次在 Microsoft Word 中打开文档 时执行查询但是 OOXML 规范建议这指定查询包含对包含实际查询的外部查询文件的引用  默认值为 false 
type: docs
weight: 110
...
不确定这个。 Microsoft Word 自动化参考建议这指定每次在 Microsoft Word 中打开文档 时执行查询。但是 OOXML 规范建议这指定查询包含对包含实际查询的外部查询文件的引用 。 默认值为` false` 。
