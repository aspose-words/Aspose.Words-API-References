...
| [ColumnStripe](../../aspose.words/tablestyle/columnstripe) { get; set; } | 获取或设置当样式指定奇数/偶数列分段时要包含在分段中的列数。 |
...
| [Shading](../../aspose.words/tablestyle/shading) { get; } | 获取一个[`Shading`](../shading)对象，该对象引用表格单元格的着色格式。 |
...
| 姓名 | 描述 |
| --- | --- |
| [Equals](../../aspose.words/style/equals)(Style) | 与指定样式比较。 样式 Istd 仅针对内置样式进行比较。 样式默认值不包括在比较中。 递归比较基本样式、链接样式和下一段样式。 |
