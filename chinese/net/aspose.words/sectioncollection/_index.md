...
Microsoft Word 文档可以包含多个部分。要在 Microsoft Word 中创建节， 选择插入/中断命令并选择中断类型。中断指定节是在新页面还是在同一页面上开始 。

以编程方式插入和删除部分可用于自定义生成的文档 在邮件合并期间。如果文档需要根据某些条件具有不同的内容或 内容的一部分，那么您可以创建一个包含 多个部分的“主”文档并删除一些邮件合并之前或之后的部分。
