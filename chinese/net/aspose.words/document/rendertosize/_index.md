...
description: Document RenderToSize 方法. 将文档页面呈现为Graphics对象到指定的大小 在 C#.
type: docs
weight: 690
...
| width | Single | 渲染页面可以占据的最大宽度（以世界单位为单位）。 |
| height | Single | 渲染页面可以占据的最大高度（以世界单位为单位）。 |

### 返回值

为呈现的页面自动计算以适合指定大小的比例。
...
using (Bitmap bmp = new Bitmap(700, 700))
{
    using (Graphics gr = Graphics.FromImage(bmp))
    {
        ...
        // 将“PageUnit”属性设置为“GraphicsUnit.Millimeter”以使用毫米作为
        // 我们将定义的任何转换和尺寸的测量单位。
