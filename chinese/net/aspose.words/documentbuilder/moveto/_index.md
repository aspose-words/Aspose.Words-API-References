...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| node | Node | 节点必须是段落或段落的直接子节点。 |

### 评论

当 节点 是一个内联级节点，光标移动到该节点 并且将在该节点之前插入更多内容。

当 节点 是 **段落** ，光标移动到段落的末尾 并且将在段落分隔符之前插入更多内容。

当 节点 是块级节点而不是段落时，光标移动到第一段的结尾插入到块级节点 中，进一步的内容将插入到段落分隔符之前。
