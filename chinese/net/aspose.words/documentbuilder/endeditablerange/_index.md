...
description: 将文档中的当前位置标记为可编辑范围结束
type: docs
weight: 210
...
文档中的可编辑范围可以重叠并跨越任何范围。要创建有效的可编辑范围，您需要 调用[`StartEditableRange`](../starteditablerange)和`EndEditableRange` 或[`EndEditableRange`](../endeditablerange)方法。
...
文档中的可编辑范围可以重叠并跨越任何范围。要创建有效的可编辑范围，您需要 调用[`StartEditableRange`](../starteditablerange)和[`EndEditableRange`](../endeditablerange) 或`EndEditableRange`方法。
