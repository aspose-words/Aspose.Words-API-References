...
| [InsertImage](../../aspose.words/documentbuilder/insertimage#insertimage_3)(Image) | 从 .NETImage:::47 插入图像:::对象到文档中。图像以 100% 的比例内嵌插入。 |
...
使用[`Font`](./font)属性指定将应用于:::47::的字符格式:从文档中的当前位置开始插入的所有文本。

使用[`ParagraphFormat`](./paragraphformat)属性为当前 指定段落格式以及将插入的所有段落。

使用[`PageSetup`](./pagesetup)属性为当前:::47 指定页面和部分属性:::部分和将插入的所有部分。
