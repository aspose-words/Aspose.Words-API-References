...
description: DocumentBuilder EndColumnBookmark 方法. 将文档中的当前位置标记为列书签结束该位置必须在表格单元格中 在 C#.
type: docs
weight: 220
...
列书签涵盖一系列行中的一个或多个列。要创建有效的书签 you 需要同时调用[`StartColumnBookmark`](../startcolumnbookmark/)和`EndColumnBookmark`与相同 **书签名称**范围。

保存文档时将忽略格式错误的书签或具有重复名称的书签。

插入的实际位置[`BookmarkEnd`](../../bookmarkend/)节点可能与当前的 document builder 位置不同。
