...
description: 如果此文档包含数字签名则返回 true 此属性仅告知文档上存在数字签名 但它不指定签名是否有效
type: docs
weight: 20
...
如果此文档包含数字签名，则返回 true。 此属性仅告知文档上存在数字签名， 但它不指定签名是否有效。
...
此属性的存在是为了帮助您对经过数字签名的文档与未经过数字签名的文档进行排序。 如果您使用 Aspose.Words 修改和保存经过数字签名的文档，则数字签名将 丢失。这是设计使然，因为存在数字签名以保护文档的真实性。 使用此属性，您可以在以与正常 文档相同的方式处理它们之前检测数字签名的文档，并采取一些措施来避免丢失数字签名，例如通知用户。
