...
description: 表示ConditionalStyle./conditionalstyle对象的集合
type: docs
weight: 310
...
表示[`ConditionalStyle`](../conditionalstyle)对象的集合。
...
| [Item](../../aspose.words/conditionalstylecollection/item) { get; } | 通过条件检索[`ConditionalStyle`](../conditionalstyle)对象风格类型。 (2 indexers) |
...
无法从此集合中添加或删除项目。它包含一组永久项目: [`ConditionalStyleType`](../conditionalstyletype)枚举类型的每个值的一个项目。
