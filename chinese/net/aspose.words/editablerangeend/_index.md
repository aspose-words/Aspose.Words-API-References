...
| [EditableRangeStart](../../aspose.words/editablerangeend/editablerangestart) { get; } | 对应的EditableRangeStart，通过ID接收。 |
...
| 姓名 | 描述 |
| --- | --- |
| override [Accept](../../aspose.words/editablerangeend/accept)(DocumentVisitor) | 接受访问者。 |
...
Word 文档中完整的可编辑范围由EditableRangeStart 和一个匹配的[`EditableRangeEnd`](../editablerangeend)具有相同的 Id。

[`EditableRangeStart`](./editablerangestart)和[`EditableRangeEnd`](../editablerangeend)是只是文档中的标记 指定可编辑范围的开始和结束位置。

使用[`EditableRange`](../editablerange)类作为“外观”来处理可编辑范围 作为单个对象。

目前仅在内联级别支持可编辑范围，即在[`Paragraph`](../paragraph),:::内47:::但可编辑范围开始和可编辑范围结束可以在不同的段落中。
