...
description: 发现 HeaderFooterCollection 中的 LinkToPrevious 方法，轻松连接或断开文档各部分之间的页眉和页脚，实现无缝格式化。
type: docs
weight: 20
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| isLinkToPrevious | Boolean | `真的`将页眉和页脚链接到上一节； `错误的`取消链接。 |
...
| isLinkToPrevious | Boolean | `真的`将页眉或页脚链接到上一节； `错误的`解除連結。 |
