---
title: Options
second_title: Aspose.Words for .NET API 参考
description: 获取或设置一组标志控制此ReportingEngineaspose.words.reporting/reportingengine实例47的行为在构建报告时
type: docs
weight: 30
url: /zh/net/aspose.words.reporting/reportingengine/options/
---
## ReportingEngine.Options property

获取或设置一组标志控制此[`ReportingEngine`](../../reportingengine)实例:::47::的行为:在构建报告时。

```csharp
public ReportBuildOptions Options { get; set; }
```

### 也可以看看

* enum [ReportBuildOptions](../../reportbuildoptions)
* class [ReportingEngine](../../reportingengine)
* namespace [Aspose.Words.Reporting](../../reportingengine)
* assembly [Aspose.Words](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Words.dll -->
