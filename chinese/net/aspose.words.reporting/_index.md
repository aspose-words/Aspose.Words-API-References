...
description: 这个命名空间包含 Aspose.Words 的新报告引擎的类 支持使用基于 LINQ 方法的语言标记的报告模板句法
type: docs
weight: 210
url: /zh/net/aspose.words.reporting/
---
这个命名空间包含 Aspose.Words 的新报告引擎的类 支持使用基于 LINQ 方法的语言标记的报告模板句法。
...
| [KnownTypeSet](./knowntypeset) | 表示包含Typeobjects 的无序集（即唯一项的集合）可以在报告模板中使用完全或部分限定名称来调用相应的 类型的静态成员、执行类型转换等。 |
| [ReportingEngine](./reportingengine) | 提供用数据填充模板文档的例程和一组控制这些例程的设置。 |
...
| [ReportBuildOptions](./reportbuildoptions) | 指定在构建报告时控制[`ReportingEngine`](../aspose.words.reporting/reportingengine)行为的选项。 |
