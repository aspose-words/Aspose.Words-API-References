...
description: 调整 ChartDataLabel Left 属性以控制图表上数据标签的位置。通过精确的距离设置增强清晰度和呈现效果。
type: docs
weight: 60
url: /zh/net/aspose.words.drawing.charts/chartdatalabel/left/
---
## ChartDataLabel.Left property

获取或设置数据标签与图表左边缘或其指定位置的距离（以点为单位）[`Position`](../position/)财产，取决于[`LeftMode`](../leftmode/) 属性.
