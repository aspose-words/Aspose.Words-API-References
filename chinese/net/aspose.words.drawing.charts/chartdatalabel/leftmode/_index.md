...
description: 探索 ChartDataLabel LeftMode 属性，自定义图表中数据标签的位置。精准控制，提升清晰度和呈现效果。
type: docs
weight: 70
url: /zh/net/aspose.words.drawing.charts/chartdatalabel/leftmode/
---
## ChartDataLabel.LeftMode property

获取或设置[`Left`](../left/)属性值：是否设置数据标签的 location 距离图表左边缘或距离其指定的位置[`Position`](../position/) 属性.
