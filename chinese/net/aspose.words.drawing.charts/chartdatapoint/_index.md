...
description: 探索 Aspose.Words.Drawing.Charts.ChartDataPoint 类，轻松格式化单个图表数据点，精确增强数据可视化。
type: docs
weight: 970
...
| 姓名 | 描述 |
| --- | --- |
| [Bubble3D](../../aspose.words.drawing.charts/chartdatapoint/bubble3d/) { get; set; } | 指定气泡图中的气泡是否应用三维效果。 |
| [Explosion](../../aspose.words.drawing.charts/chartdatapoint/explosion/) { get; set; } | 指定数据点应从饼图中心移动的量。 可以为负数，负数表示未设置属性，不应应用爆炸。 仅适用于饼图。 |
...
| [InvertIfNegative](../../aspose.words.drawing.charts/chartdatapoint/invertifnegative/) { get; set; } | 指定当值为负时父元素是否应反转其颜色。 |
...
在一个系列中，`ChartDataPoint`对象是[`ChartDataPointCollection`](../chartdatapointcollection/) 该[`ChartDataPointCollection`](../chartdatapointcollection/)包含一个`ChartDataPoint`每个点的对象。
