...
description: 使用 DigitalSignatureCollection 的 IsValid 属性确保文档完整性。该属性会验证所有签名是否有效且未被篡改，从而增强安全性。
type: docs
weight: 30
...
返回`真的`如果此集合中的所有数字签名均有效且文档未被篡改 还返回`真的`如果没有数字签名。 返回`错误的`如果至少一个数字签名无效。
