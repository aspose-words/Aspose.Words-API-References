...
description: LayoutEnumerator MoveNext 方法. 按视觉顺序移动到下一个同级实体 当迭代跨页的段落行时此方法 不会移动到下一页而是移动到同一页面上的下一个实体 在 C#.
type: docs
weight: 120
...
按视觉顺序移动到下一个同级实体。 当迭代跨页的段落行时，此方法 不会移动到下一页，而是移动到同一页面上的下一个实体。
