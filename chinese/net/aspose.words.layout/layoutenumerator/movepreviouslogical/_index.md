...
description: LayoutEnumerator MovePreviousLogical 方法. 按逻辑顺序移至上一个同级实体 当迭代跨页中断的段落的行时此方法 将移至上一行即使它位于另一页上 在 C#.
type: docs
weight: 160
...
按逻辑顺序移至上一个同级实体。 当迭代跨页中断的段落的行时，此方法 将移至上一行，即使它位于另一页上。
...
注意所有Span实体被链接在一起，因此如果[`Current`](../current/) 实体是跨度重复调用此方法将迭代文档的完整故事。
