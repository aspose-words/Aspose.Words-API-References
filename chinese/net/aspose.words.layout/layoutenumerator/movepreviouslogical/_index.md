...
description: 按逻辑顺序移动到上一个兄弟实体 当迭代跨页中断的段落行时此方法 将移动到上一行即使它位于另一页上
type: docs
weight: 150
...
按逻辑顺序移动到上一个兄弟实体。 当迭代跨页中断的段落行时，此方法 将移动到上一行，即使它位于另一页上。
...
注意所有Span实体链接在一起，因此如果[`Current`](../current) 实体是跨度重复调用此方法将迭代文档的完整故事。
