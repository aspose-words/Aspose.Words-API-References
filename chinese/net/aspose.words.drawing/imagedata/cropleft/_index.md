...
description: 探索 ImageData CropLeft 属性，通过从左侧删除不需要的部分来实现完美适配，轻松调整图像框架。
type: docs
weight: 70
url: /zh/net/aspose.words.drawing/imagedata/cropleft/
---
...
裁剪量的范围为 -1.0 到 1.0。默认值为 0。请注意，值为 1 时将完全不显示任何图片。负值会导致图片从被裁剪的边缘向内挤压（图片与裁剪边缘之间的空白区域将被形状的填充颜色填充）。小于 1 的正值会导致剩余图片被拉伸以适应形状。
