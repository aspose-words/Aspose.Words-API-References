...
description: SignatureLine Instructions 财产. 获取或设置在签署签名行时显示给签名者的指令 如果出现以下情况则忽略此属性DefaultInstructions已设置 此属性的默认值为空字符串Empty  在 C#.
type: docs
weight: 50
...
获取或设置在签署签名行时显示给签名者的指令。 如果出现以下情况，则忽略此属性[`DefaultInstructions`](../defaultinstructions/)已设置。 此属性的默认值为**空字符串**(Empty ).
