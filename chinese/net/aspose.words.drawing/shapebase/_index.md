...
description: Aspose.Words.Drawing.ShapeBase 班级. 绘图层中对象的基类例如自选图形自由格式OLE 对象ActiveX 控件或图片 在 C#.
type: docs
weight: 1260
...
如果该形状是 a 的子代[`Paragraph`](../../aspose.words/paragraph/)对象，则该形状被称为“顶级”。 顶级形状以点为单位进行测量和定位。

形状也可以作为[`GroupShape`](../groupshape/)当多个shapes 分组时的对象。组形状的子形状位于坐标空间中，并且units 由[`CoordSize`](./coordsize/)和[`CoordOrigin`](./coordorigin/)Parent 组形状的属性。

形状可以与文本内嵌或浮动放置。定位方法是controlled ，使用[`WrapType`](./wraptype/)财产。

当形状浮动时，它是相对于某些东西定位的（例如当前段落、 边距或页面）。形状的相对定位使用 the 指定[`RelativeHorizontalPosition`](./relativehorizontalposition/)和[`RelativeVerticalPosition`](./relativeverticalposition/)特性。

使用以下命令显式定位浮动形状[`Left`](./left/)和[`Top`](./top/) 属性或使用以下方法相对于某些其他对象对齐[`HorizontalAlignment`](./horizontalalignment/) 和[`VerticalAlignment`](./verticalalignment/)特性。
