...
description: 绘图层中对象的基类例如自选图形自由格式OLE 对象ActiveX 控件或图片
type: docs
weight: 1090
...
如果形状是[`Paragraph`](../../aspose.words/paragraph)对象的子对象，则称该形状为“顶部-等级”。 顶级形状以点为单位进行测量和定位。

当有多个形状:::时，形状也可以作为[`GroupShape`](../groupshape)对象的子对象出现:47:::被分组。组形状的子形状位于坐标空间中，单位 由[`CoordSize`](./coordsize)和P定义:Aspose.Words.Drawing.ShapeBase.CoordOrigin父级 组形状的属性。

形状可以与文本内嵌或浮动定位。使用[`WrapType`](./wraptype)属性控制定位方法 。

当一个形状浮动时，它相对于某物（例如当前段落、 边距或页面）定位。使用 [`RelativeHorizontalPosition`](./relativehorizontalposition)和RelativeVerticalPosition属性。

使用[`Left`](./left)和P显式定位浮动形状:Aspose.Words.Drawing.ShapeBase.Top 属性或使用[`HorizontalAlignment`](./horizontalalignment)相对于其他对象对齐 和[`VerticalAlignment`](./verticalalignment)属性。
