...
description: 可用于在将文档保存为XamlFixed格式时指定其他选项
type: docs
weight: 5360
...
| [ResourcesFolder](../../aspose.words.saving/xamlfixedsaveoptions/resourcesfolder) { get; set; } | 指定将文档导出为固定页面 Xaml 格式时保存资源（图像和字体）的物理文件夹。 默认为` null` 。 |
| [ResourcesFolderAlias](../../aspose.words.saving/xamlfixedsaveoptions/resourcesfolderalias) { get; set; } | 指定用于构造写入固定页面 Xaml 文档的图像 URI 的文件夹的名称。 默认为` null` 。 |
| override [SaveFormat](../../aspose.words.saving/xamlfixedsaveoptions/saveformat) { get; set; } | 如果使用此保存选项对象，则指定保存文档的格式。 只能是XamlFixed。 |
