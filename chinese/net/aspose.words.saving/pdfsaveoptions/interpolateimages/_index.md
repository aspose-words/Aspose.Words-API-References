...
description: 指示图像插值是否应由合格阅读器执行的标志 当 false 指定时标志不会写入输出文档和 reader 的默认行为改为使用
type: docs
weight: 180
...
指示图像插值是否应由合格阅读器执行的标志。 当` false` 指定时，标志不会写入输出文档和 reader 的默认行为改为使用。
...
当源图像的分辨率明显低于该分辨率时输出设备的 每个源样本都覆盖了许多设备像素。因此，图像可能会出现锯齿状或块状。 这些视觉伪影可以通过在渲染过程中应用图像插值算法来减少。 图像插值 不是用相同的颜色绘制源样本覆盖的所有像素，而是尝试在相邻样本值之间产生平滑过渡。

符合标准的阅读器可以选择不实现 PDF 的这个特性 或者可以使用它希望的任何特定的插值实现。

默认值为` false` 。

PDF/A 合规性禁止插值标志。` false` 值将在保存到 PDF/A 时自动使用 。
