...
description: 获取或设置确定 PDF 文档中 JPEG 图像质量的值
type: docs
weight: 190
...
此属性与[`ImageCompression`](../imagecompression)选项结合使用。

仅在文档包含 JPEG 图像时有效。

当保存为 PDF 格式时，使用此属性获取或设置文档中图像的质量。 该值可能在 0 到 100 之间变化，其中 0 表示质量最差但压缩率最高，100 表示质量最好但压缩率最低。 如果质量为 100 且源图像为 JPEG，则表示不压缩 - 将保存原始字节。
