...
description: 控制字体如何嵌入到生成的 PDF 文档中
type: docs
weight: 100
...
默认值为` false` ，这意味着字体在嵌入之前被子集化。 如果您想保持输出文件更小，子集很有用。子集从字体中删除所有 未使用的字形。

当此值设置为` true` 时，将完整的字体文件嵌入到 PDF 中，而无需 子集。这将导致更大的输出文件，但当您想要 稍后编辑生成的 PDF（例如添加更多文本）时，这可能是一个有用的选项。

一些字体很大（几兆字节）并且在没有子集的情况下嵌入它们 会导致输出文档很大。
