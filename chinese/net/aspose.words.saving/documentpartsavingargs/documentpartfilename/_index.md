...
description: 获取或设置文档部分将保存到的文件名不带路径
type: docs
weight: 20
...
此属性允许您重新定义文档部分文件名的生成方式 在导出到 HTML 或 EPUB 期间。

调用回调时，此属性包含由 Aspose.Words 生成 的文件名。您可以更改此属性的值以将文档部分保存到 不同的文件中。请注意，每个部分的文件名必须是唯一的。

`DocumentPartFileName`必须只包含文件名而不包含路径。 Aspose.Words 使用文档文件名确定保存路径。如果未指定输出文档 文件名，例如保存到流时，则此文件名仅用于 引用文档部分。保存为 EPUB 格式时也是如此。
