...
默认值为`无效的` 。当这个属性是`无效的` ，文档部分 将被保存到指定的文件中[`DocumentPartFileName`](../documentpartfilename/)财产。

当请求以 HTML 格式保存到流时[`Save`](../../../aspose.words/document/save/) 或[`Save`](../../../aspose.words/document/save/)第一个文档部分即将保存， Aspose.Words 在此建议调用者最初传递的主输出流。
