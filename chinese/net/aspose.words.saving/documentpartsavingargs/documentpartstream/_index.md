...
description: DocumentPartSavingArgs DocumentPartStream 财产. 允许指定将文档部分保存到的流 在 C#.
type: docs
weight: 30
...
默认值为`无效的` .当这个属性是`无效的` ，文档部分 将保存到指定的文件中[`DocumentPartFileName`](../documentpartfilename/)财产。

当请求以 HTML 格式保存到流时[`Save`](../../../aspose.words/document/save/) 或[`Save`](../../../aspose.words/document/save/)第一个文档部分即将被保存， Aspose.Words 在这里建议调用者最初传递的主要输出流。

保存为基于 HTML 的容器格式 EPUB 格式时，`DocumentPartStream`不能指定 ，因为所有附属部分都将封装到单个输出包中。
