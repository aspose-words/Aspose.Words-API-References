...
description: 了解 DocumentPartSavingArgs 中的 DocumentPartStream 属性如何让您轻松指定保存文档部分的位置以实现无缝管理。
type: docs
weight: 30
...
默认值为`无效的` . 当此属性`无效的` ，文档部分 将保存到[`DocumentPartFileName`](../documentpartfilename/)财产。

当请求以 HTML 格式保存到流时[`Save`](../../../aspose.words/document/save/) 或[`Save`](../../../aspose.words/document/save/)并且第一个文档部分即将被保存， Aspose.Words 在这里建议调用者最初传递的主输出流。

当保存为基于 HTML 的容器格式 EPUB 格式时，`DocumentPartStream`无法指定 ，因为所有附属部分都将被封装到单个输出包中。
