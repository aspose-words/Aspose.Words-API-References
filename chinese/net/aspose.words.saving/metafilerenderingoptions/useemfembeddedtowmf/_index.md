...
获取或设置一个值，确定如何呈现嵌入 EMF 图元文件的 WMF 图元文件。
...
当该值设置为`真的`，Aspose.Words 在渲染时使用嵌入的 EMF 数据。

当该值设置为`错误的`，Aspose.Words在渲染时使用WMF数据。

仅当图元文件呈现为矢量图形时才使用此选项。当图元文件渲染 位图时，始终使用 WMF 数据。
