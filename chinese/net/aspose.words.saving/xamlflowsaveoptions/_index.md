...
description: 可用于在将文档保存到 XamlFlow时指定其他选项或XamlFlowPack格式
type: docs
weight: 5370
...
可用于在将文档保存到 XamlFlow时指定其他选项或XamlFlowPack格式。
...
| 姓名 | 描述 |
| --- | --- |
| [XamlFlowSaveOptions](xamlflowsaveoptions#constructor)() | 初始化此类的新实例，该实例可用于以XamlFlow格式保存文档. |
| [XamlFlowSaveOptions](xamlflowsaveoptions#constructor_1)(SaveFormat) | 初始化此类的新实例，该实例可用于将文档保存在XamlFlow中 或XamlFlowPack格式。 |
...
| [ImageSavingCallback](../../aspose.words.saving/xamlflowsaveoptions/imagesavingcallback) { get; set; } | 允许控制在将文档保存到 XAML 时如何保存图像。 |
...
| override [SaveFormat](../../aspose.words.saving/xamlflowsaveoptions/saveformat) { get; set; } | 如果使用此保存选项对象，则指定保存文档的格式。 只能是XamlFlow。 |
