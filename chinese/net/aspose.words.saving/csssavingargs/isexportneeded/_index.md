...
description: 使用 CssSavingArgs 的 IsExportNeeded 属性控制 CSS 导出。选择将 CSS 嵌入或保存到文件以优化 HTML 文档。默认值为 true。
type: docs
weight: 30
...
允许指定是否将 CSS 导出到文件并嵌入到 HTML 文档。默认为`真的`. 当此属性`错误的`，CSS 信息将不会保存到 CSS 文件中，也不会嵌入到 HTML 文档中。
