...
description: Aspose.Words.Saving.TxtSaveOptions 班级. 可用于在将文档保存到Text格式 在 C#.
type: docs
weight: 5660
...
| [ListIndentation](../../aspose.words.saving/txtsaveoptions/listindentation/) { get; } | 获得[`TxtListIndentation`](../txtlistindentation/)指定用于列表级别缩进的字符数和哪个字符的对象。 默认情况下，字符 '\0' 的计数为零，这意味着没有缩进。 |
| [MaxCharactersPerLine](../../aspose.words.saving/txtsaveoptions/maxcharactersperline/) { get; set; } | 获取或设置一个整数值，指定每一行的最大字符数。 默认值为 0，表示没有限制。 |
...
| [PreserveTableLayout](../../aspose.words.saving/txtsaveoptions/preservetablelayout/) { get; set; } | 指定程序在以纯文本格式保存时是否应尝试保留表的布局。 默认值为`错误的`. |
...
| override [SaveFormat](../../aspose.words.saving/txtsaveoptions/saveformat/) { get; set; } | 指定使用此保存选项对象时保存文档的格式。 只能是Text. |
| [SimplifyListLabels](../../aspose.words.saving/txtsaveoptions/simplifylistlabels/) { get; set; } | 指定程序是否应在 复杂标签格式无法用纯文本充分表示的情况下简化列表标签。 |
