...
description: 指定程序是否应在 复杂的标签格式不能用纯文本充分表示的情况下简化列表标签
type: docs
weight: 70
...
指定程序是否应在 复杂的标签格式不能用纯文本充分表示的情况下简化列表标签。

如果设置为 **true** ，编号列表标签以简单数字格式写入 和逐项列表标签作为简单的 ASCII 字符。默认值为 **false** 。
...
TxtSaveOptions txtSaveOptions = new TxtSaveOptions();
 ...
 // 将“SimplifyListLabels”属性设置为“true”以转换一些 list
