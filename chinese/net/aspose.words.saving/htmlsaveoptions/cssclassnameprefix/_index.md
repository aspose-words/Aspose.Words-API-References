...
description: HtmlSaveOptions CssClassNamePrefix 财产. 指定添加到所有 CSS 类名的前缀 默认值为空字符串生成的 CSS 类名没有公共前缀 在 C#.
type: docs
weight: 30
...
如果此值不为空，Aspose.Words 生成的所有 CSS 类都将以指定的前缀开头。 这可能很有用，例如，如果您将自定义 CSS 添加到生成的文档并希望防止 class 名称冲突。

如果值不是`无效的`或为空，它必须是有效的 CSS 标识符。

## 例子

展示如何将文档保存为 HTML，并为其所有 CSS 类名称添加前缀。
