...
description: 如果您想在页面布局模型的构建和呈现期间调用自己的自定义方法，请实现此接口。
type: docs
weight: 653
...
如果您想在页面布局模型的构建和呈现期间调用自己的自定义方法，请实现此接口。此接口的主要用途是允许应用程序代码中止构建过程。
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| args | [PageLayoutCallbackArgs](../../com.aspose.words/pagelayoutcallbackargs) | 事件的一个参数。实现中止布局构建过程时抛出异常。 |
