...
**退货：**
 int - 生成图像的颜色模式。返回值是其中之一[ImageColorMode](../../com.aspose.words/imagecolormode)常数。
...
什么时候[MetafileRenderingMode.VECTOR](../../com.aspose.words/metafilerenderingmode\#VECTOR)指定后，Aspose.Words首先使用自己的图元文件渲染引擎将图元文件渲染为矢量图形，然后再将矢量图形渲染为图像。

什么时候[MetafileRenderingMode.BITMAP](../../com.aspose.words/metafilerenderingmode\#BITMAP)指定后，Aspose.Words 使用 GDI+ 图元文件渲染引擎将图元文件直接渲染到图像中。

GDI+ 图元文件渲染引擎运行速度更快，支持几乎所有图元文件功能，但与页面上的其余矢量图形（尤其是文本）相比，低分辨率可能会产生不一致的结果。 Aspose.Words 图元文件渲染引擎即使在低分辨率下也会产生更一致的结果，但工作速度较慢并且可能不准确地渲染复杂的图元文件。
...
此属性仅在呈现文档页面时有效。在将形状渲染为图像时忽略此属性。
...
当呈现指定其自身背景颜色的文档页面时，文档背景颜色将覆盖此属性指定的颜色。
...
由于 GDI+ 的工作，输出图像的像素格式可能与设置值不同。

**退货：**
int - 生成图像的像素格式。返回值是其中之一[ImagePixelFormat](../../com.aspose.words/imagepixelformat)常数。
...
**退货：**
int - 将图像转换为 1 bpp 格式时使用的方法[getSaveFormat()](../../com.aspose.words/imagesaveoptions\#getSaveFormat--) / [setSaveFormat(int)](../../com.aspose.words/imagesaveoptions\#setSaveFormat-int-)是 SaveFormat.Tiff 和[getTiffCompression()](../../com.aspose.words/imagesaveoptions\#getTiffCompression--) / [setTiffCompression(int)](../../com.aspose.words/imagesaveoptions\#setTiffCompression-int-)等于 TiffCompression.Ccitt3 或 TiffCompression.Ccitt4。返回值是其中之一[ImageBinarizationMethod](../../com.aspose.words/imagebinarizationmethod)常数。
...
**退货：**
 int - 将生成的图像保存为 TIFF 格式时应用的压缩类型。返回值是其中之一[TiffCompression](../../com.aspose.words/tiffcompression)常数。
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| value | byte | 决定 Floyd-Steinberg 方法中二值化误差值的阈值。 |
