...
使用 TA 字段指定的条目构建权限表（即，法律文件中的引用列表，例如对案例、法规和规则的引用，以及引用出现的页码） .
## 方法
...
| [getUsePassim()](#getUsePassim--) | 获取是否用“passim”替换对同一权威的五个或更多不同页面引用，“passim”用于表示某个词或段落在引用的作品中频繁出现。 |
| [hashCode()](#hashCode--) |  |
...
| [setRemoveEntryFormatting(boolean value)](#setRemoveEntryFormatting-boolean-) | 设置是否从引文目录中的条目中删除文档中条目文本的格式。 |
...
| [setUseHeading(boolean value)](#setUseHeading-boolean-) | 设置是否在引文目录中包含条目的类别标题。 |
| [setUsePassim(boolean value)](#setUsePassim-boolean-) | 设置是否用“passim”替换对同一权威的五个或更多不同页面引用，用于表示某个词或段落在引用的作品中频繁出现。 |
| [toString()](#toString--) |  |
...
**退货:**
java.lang.String - 用于分隔目录条目及其页码的字符序列。
...
**退货:**
boolean - 是否从引文目录中的条目中删除文档中条目文本的格式。
### getResult() {#getResult--}
```
...
获取是否用“passim”替换对同一权威的五个或更多不同页面引用，“passim”用于表示某个词或段落在引用的作品中频繁出现。
...
设置是否用“passim”替换对同一权威的五个或更多不同页面引用，用于表示某个词或段落在引用的作品中频繁出现。
