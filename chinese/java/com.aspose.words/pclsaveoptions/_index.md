...
| [setRasterizeTransformedElements(boolean value)](#setRasterizeTransformedElements-boolean-) | 设置一个值，确定在保存到 PCL 文档之前是否应对复杂的变换元素进行光栅化。 |
...
获取一个值，该值确定在保存到 PCL 文档之前是否应对复杂的转换元素进行光栅化。默认为 true 。 PCL 不支持 Aspose Words 使用的某种转换。例如旋转、倾斜的图像和纹理画笔。为了正确渲染这些元素，使用了光栅化过程，即保存到图像和剪辑。此过程可能需要额外的时间和内存。如果 flag 设置为 false ，输出中的某些内容可能与源文档不同。

**退货：**
布尔值 - 确定在保存到 PCL 文档之前是否应光栅化复杂变换元素的值。
...
设置一个值，确定在保存到 PCL 文档之前是否应对复杂的变换元素进行光栅化。默认为 true 。 PCL 不支持 Aspose Words 使用的某种转换。例如旋转、倾斜的图像和纹理画笔。为了正确渲染这些元素，使用了光栅化过程，即保存到图像和剪辑。此过程可能需要额外的时间和内存。如果 flag 设置为 false ，输出中的某些内容可能与源文档不同。

**参数：**

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| value | boolean | 确定在保存到 PCL 文档之前是否应光栅化复杂变换元素的值。 |
