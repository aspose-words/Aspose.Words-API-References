...
description: "Java 中的 LINQ Reporting Engine 上下文。"
type: docs
weight: 572
url: /zh/java/com.aspose.words/reportbuildercontext/
---
...
| 构造函数 | 描述 |
| --- | --- |
| [ReportBuilderContext()](#ReportBuilderContext) | 初始化此类的新实例。 |
## 方法

| 方法 | 描述 |
| --- | --- |
| [getDataSources()](#getDataSources) | 用于生成报告的数据源。 |
| [getFontSettings()](#getFontSettings) | 处理器使用的字体设置。 |
| [getLayoutOptions()](#getLayoutOptions) | 处理器使用的文档布局选项。 |
| [getReportBuilderOptions()](#getReportBuilderOptions) | 报告构建选项。 |
...
键表示数据源，而值是数据源名称。数据源名称可以为 null 或空字符串；在这种情况下，报告引擎将自动从指定的数据源检测数据源名称。
...
**Returns:**
java.util.LinkedHashMap - 对应的 java.util.HashMap 值。
