...
**HeaderFooter**可以包含**Paragraph**和**Table**子节点。

**HeaderFooter**是节级节点，只能是**Section**.只能有一个**HeaderFooter**或每个[getHeaderFooterType()](../../com.aspose.words/headerfooter\#getHeaderFooterType--)在一个**Section**.

如果**Section**没有**HeaderFooter**特定类型或**HeaderFooter**没有子节点，此页眉/页脚被视为链接到 Microsoft Word 中上一节相同类型的页眉/页脚。

什么时候**HeaderFooter**至少包含一个**Paragraph**它不再被视为链接到 Microsoft Word 中的上一个。
## 构造器

| 构造函数 | 描述 |
| --- | --- |
| [HeaderFooter(DocumentBase doc, int headerFooterType)](#HeaderFooter-com.aspose.words.DocumentBase-int-) | 初始化此类的新实例。 |
## 方法
...
| [getHeaderFooterType()](#getHeaderFooterType--) | 获取此页眉/页脚的类型。 |
...
| [getNodeType()](#getNodeType--) | 退货**NodeType.HeaderFooter**. |
...
| [isHeader()](#isHeader--) | 如果这个是真的**HeaderFooter**对象是标题。 |
| [isLinkedToPrevious()](#isLinkedToPrevious--) | 如果此页眉或页脚链接到上一节中相应的页眉或页脚，则为真。 |
| [isLinkedToPrevious(boolean value)](#isLinkedToPrevious-boolean-) | 如果此页眉或页脚链接到上一节中相应的页眉或页脚，则为真。 |
...
**退货：**
boolean - 如果所有节点都被访问，则为真；如果 DocumentVisitor 在访问所有节点之前停止操作，则返回 false。调用 DocumentVisitor.VisitHeaderFooterStart，然后为该部分的所有子节点调用 Accept，最后调用 DocumentVisitor.VisitHeaderFooterEnd。
...
**退货：**
 int - 此页眉/页脚的类型。返回值是其中之一[HeaderFooterType](../../com.aspose.words/headerfootertype)常数。
...
退货**NodeType.HeaderFooter**.

**退货：**
整数 -**NodeType.HeaderFooter** .返回值是其中之一[NodeType](../../com.aspose.words/nodetype)常数。
