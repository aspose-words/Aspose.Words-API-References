...
提供使用连字字典的方法。这些词典规定了特定语言的单词可以在何处连字符。
...
| [getCallback()](#getCallback--) | 获取用于在构建文档页面布局时请求字典的回调接口。 |
| [getClass()](#getClass--) |  |
...
| [setCallback(IHyphenationCallback value)](#setCallback-com.aspose.words.IHyphenationCallback-) | 设置用于在构建文档页面布局时请求字典的回调接口。 |
...
获取用于在构建文档页面布局时请求字典的回调接口。这允许延迟加载字典，这在处理多种语言的文档时可能很有用。
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| language | java.lang.String | 语言名称，例如“en-US”。有关详细信息，请参阅“区域性名称”的 .NET 文档和 RFC 4646。 |
| fileName | java.lang.String | Open Office 格式的字典文件的路径。

如果此参数为 null 或空字符串，则注册为 Null 字典，并且不再为该语言调用回调。
...
设置用于在构建文档页面布局时请求字典的回调接口。这允许延迟加载字典，这在处理多种语言的文档时可能很有用。

**参数：**

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| value | [IHyphenationCallback](../../com.aspose.words/ihyphenationcallback) | 用于在构建文档页面布局时请求词典的回调接口。 |
