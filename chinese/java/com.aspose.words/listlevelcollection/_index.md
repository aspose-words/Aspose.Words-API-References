---
title: ListLevelCollection
second_title: Aspose.Words for Java API 参考
description: 列表中每个级别的列表格式的集合。
type: docs
weight: 374
url: /zh/java/com.aspose.words/listlevelcollection/
---

**遗产：**
java.lang.Object

**所有已实现的接口：**
java.lang.Cloneable, java.lang.Iterable
```
public class ListLevelCollection implements Cloneable, Iterable
```

列表中每个级别的列表格式的集合。

要了解更多信息，请访问**Working with Lists**文档文章。
## 方法

| 方法 | 描述 |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [get(int index)](#get-int-) | 按索引获取列表级别。 |
| [getClass()](#getClass--) |  |
| [getCount()](#getCount--) | 获取此列表中的级别数。 |
| [hashCode()](#hashCode--) |  |
| [iterator()](#iterator--) | 获取将枚举此列表中级别的枚举器对象。 |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [set(int index, ListLevel value)](#set-int-com.aspose.words.ListLevel-) | 按索引获取列表级别。 |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**参数：**

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**退货：**
布尔值
### get(int index) {#get-int-}
```
public ListLevel get(int index)
```


按索引获取列表级别。

**参数：**

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| index | int |  |

**退货：**
[ListLevel](../../com.aspose.words/listlevel) - 按索引列出级别。
### getClass() {#getClass--}
```
public final native Class<?> getClass()
```




**退货：**
java.lang.Class<?>
### getCount() {#getCount--}
```
public int getCount()
```


获取此列表中的级别数。

列表中可能有 1 个或 9 个级别。

**退货：**
int - 此列表中的级别数。
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**退货：**
整数
### iterator() {#iterator--}
```
public Iterator iterator()
```


获取将枚举此列表中级别的枚举器对象。

**退货：**
java.util.迭代器
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### set(int index, ListLevel value) {#set-int-com.aspose.words.ListLevel-}
```
public void set(int index, ListLevel value)
```


按索引获取列表级别。

**参数：**

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| index | int |  |
| value | [ListLevel](../../com.aspose.words/listlevel) | 按索引排列的列表级别。 |

### toString() {#toString--}
```
public String toString()
```




**退货：**
java.lang.字符串
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**参数：**

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**参数：**

| 范围 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |
