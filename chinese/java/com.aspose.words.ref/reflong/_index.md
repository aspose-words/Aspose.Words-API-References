---
title: RefLong
second_title: Aspose.Words for Java API 参考
description:表示 Long 原始类型的 Out/Ref 构造
type: docs
weight: 16
url: /zh/java/com.aspose.words.ref/reflong/
---

**遗产:**
java.lang.Object
```
public class RefLong
```

表示 Long 原始类型的 Out/Ref 构造
## 构造函数s

| 构造函数 | 描述 |
| --- | --- |
| [RefLong(long value)](#RefLong-long-) | 使用给定值模拟 Out/Ref 参数初始化。 |
## 方法s

| 方法 | 描述 |
| --- | --- |
| [equals(Object arg0)](#equals-java.lang.Object-) |  |
| [get()](#get--) | 获取 Out/Ref 参数值 应在调用具有 Out/Ref 构造的方法后使用 |
| [get班级()](#get班级--) |  |
| [hashCode()](#hashCode--) |  |
| [notify()](#notify--) |  |
| [notifyAll()](#notifyAll--) |  |
| [set(long anotherValue)](#set-long-) | 设置 Out/Ref 参数值 应在具有 Out/Ref 结构的方法内部使用 |
| [toString()](#toString--) |  |
| [wait()](#wait--) |  |
| [wait(long arg0)](#wait-long-) |  |
| [wait(long arg0, int arg1)](#wait-long-int-) |  |
### RefLong(long value) {#RefLong-long-}
```
public RefLong(long value)
```


使用给定值模拟 Out/Ref 参数初始化。应该在调用具有 Out/Ref 结构的方法之前使用

**参数:**
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| value | long |  |

### equals(Object arg0) {#equals-java.lang.Object-}
```
public boolean equals(Object arg0)
```




**参数:**
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | java.lang.Object |  |

**退货:**
布尔值
### get() {#get--}
```
public long get()
```


获取 Out/Ref 参数值 应在调用具有 Out/Ref 构造的方法后使用

**退货:**
长
### get班级() {#get班级--}
```
public final native 班级<?> get班级()
```




**退货:**
java.lang.班级<?>
### hashCode() {#hashCode--}
```
public native int hashCode()
```




**退货:**
整数
### notify() {#notify--}
```
public final native void notify()
```




### notifyAll() {#notifyAll--}
```
public final native void notifyAll()
```




### set(long anotherValue) {#set-long-}
```
public long set(long anotherValue)
```


设置 Out/Ref 参数值 应在具有 Out/Ref 结构的方法内部使用

**参数:**
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| anotherValue | long |  |

**退货:**
长
### toString() {#toString--}
```
public String toString()
```




**退货:**
java.lang.String
### wait() {#wait--}
```
public final void wait()
```




### wait(long arg0) {#wait-long-}
```
public final native void wait(long arg0)
```




**参数:**
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | long |  |

### wait(long arg0, int arg1) {#wait-long-int-}
```
public final void wait(long arg0, int arg1)
```




**参数:**
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| arg0 | long |  |
| arg1 | int |  |
