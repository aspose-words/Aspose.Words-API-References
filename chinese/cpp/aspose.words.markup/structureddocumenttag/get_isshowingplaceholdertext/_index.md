...
description: "Aspose::Words::Markup::StructuredDocumentTag::get_IsShowingPlaceholderText method. 指定此 SDT 的内容是否应被解释为包含占位符文本（而不是 SDT 内的常规文本内容）。如果设置为 true，则在 C++ 中打开此文档时将恢复此状态（显示占位符文本）。"
type: docs
weight: 18000
url: /zh/cpp/aspose.words.markup/structureddocumenttag/get_isshowingplaceholdertext/
---
## StructuredDocumentTag::get_IsShowingPlaceholderText method


指定此 **SDT** 的内容是否应被解释为包含占位符文本（而不是 SDT 中的常规文本内容）。如果设置为 **true**，则在打开此文档时将恢复此状态（显示占位符文本）。
