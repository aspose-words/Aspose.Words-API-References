...
description: Entdecken Sie die CurrentNode-Eigenschaft von DocumentBuilder, um einfach auf den ausgewählten Knoten zuzugreifen und so die Effizienz und den Arbeitsablauf Ihrer Dokumentbearbeitung zu verbessern.
type: docs
weight: 40
...
Ruft den Knoten ab, der derzeit in diesem DocumentBuilder ausgewählt ist.
...
`CurrentNode` ist ein Cursor von[`DocumentBuilder`](../) und weist auf eine[`Node`](../../node/) , das ein direktes Kind von einem[`Paragraph`](../../paragraph/) . Alle Einfügevorgänge, die Sie mit durchführen[`DocumentBuilder`](../) wird eingefügt vor dem`CurrentNode`.

Wenn der aktuelle Absatz leer ist oder der Cursor direkt vor dem Ende eines Absatzes oder eines strukturierten Dokument-Tags positioniert ist,`CurrentNode` Rücksendungen`null`.
