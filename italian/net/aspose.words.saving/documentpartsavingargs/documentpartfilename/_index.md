...
description: Ottiene o imposta il nome del file senza percorso in cui verrà salvata la parte del documento.
type: docs
weight: 20
...
Questa proprietà consente di ridefinire il modo in cui i nomi dei file delle parti del documento vengono generati durante l'esportazione in HTML o EPUB.

Quando viene richiamato il callback, questa proprietà contiene il nome del file che è stato generato da Aspose.Words. È possibile modificare il valore di questa proprietà per salvare la parte del documento in un file diverso. Si noti che il nome del file per ciascuna parte deve essere univoco.

`DocumentPartFileName` deve contenere solo il nome del file senza il percorso. Aspose.Words determina il percorso per il salvataggio utilizzando il nome del file del documento. Se il nome file del documento di output non è stato specificato, ad esempio durante il salvataggio in uno stream, questo nome file viene utilizzato solo per fare riferimento a parti del documento. Lo stesso vale quando si salva in formato EPUB.
