...
description: Scopri la proprietà StatusText di FormField per personalizzare i messaggi della barra di stato quando i campi del modulo sono attivati. Migliora l'esperienza utente senza sforzo!
type: docs
weight: 180
...
Restituisce o imposta il testo visualizzato nella barra di stato quando un campo del modulo è attivo.
...
Se il[`OwnStatus`](../ownstatus/) la proprietà è impostata su`VERO` , IL`StatusText` la proprietà specifica il testo della barra di stato. Se[`OwnStatus`](../ownstatus/) la proprietà è impostata su`falso` , IL`StatusText` La proprietà specifica il nome di una voce AutoText che contiene il testo della barra di stato per il campo del modulo.

Microsoft Word consente stringhe composte da un massimo di 138 caratteri.
