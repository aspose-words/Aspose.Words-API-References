...
description: Inserisce una stringa HTML nel documento.
type: docs
weight: 330
...
| --- | --- | --- |
| html | String | Una stringa HTML da inserire nel documento. |
| useBuilderFormatting | Boolean | Un valore che indica se la formattazione è specificata in[`DocumentBuilder`](../../documentbuilder) viene utilizzato come formattazione di base per il testo importato da HTML. |

### Osservazioni

Puoi usare questo metodo per inserire un frammento HTML o un intero documento HTML.

Quando*useBuilderFormatting* è`falso` , [`DocumentBuilder`](../../documentbuilder) la formattazione viene ignorata e la formattazione del testo inserito si basa sulla formattazione HTML predefinita. Di conseguenza, il testo appare come viene visualizzato nei browser.

Quando*useBuilderFormatting* è`VERO` , si basa sulla formattazione del testo inserito[`DocumentBuilder`](../../documentbuilder) formattazione, e il testo appare come se fosse stato inserito con[`Write`](../write) .
