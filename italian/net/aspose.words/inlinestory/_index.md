...
description: Scopri la classe Aspose.Words.InlineStory, la base essenziale per i nodi in linea, che consente l'integrazione perfetta di paragrafi e tabelle nei tuoi documenti.
type: docs
weight: 3720
...
Classe base per nodi di livello inline che possono contenere paragrafi e tabelle.
...
| abstract [StoryType](../../aspose.words/inlinestory/storytype/) { get; } | Restituisce il tipo di storia. |
...
Le classi che derivano da`InlineStory` sono nodi a livello inline che possono contenere il proprio testo (paragrafi e tabelle). Ad esempio, un[`Comment`](../comment/) il nodo contiene il testo di un commento e un[`Footnote`](../../aspose.words.notes/footnote/) contiene il testo di una nota a piè di pagina.
