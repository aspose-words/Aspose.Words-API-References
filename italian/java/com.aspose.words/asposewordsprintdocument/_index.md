...
description: "Fornisce un'implementazione predefinita per la stampa di un Document all'interno del framework di stampa Java."
type: docs
weight: 20
url: /it/java/com.aspose.words/asposewordsprintdocument/
---
...
Fornisce un'implementazione predefinita per la stampa di un [Document](../../com.aspose.words/document/) all'interno del framework di stampa Java.

Per saperne di più, visita l'articolo di documentazione [ Printing a Document Programmatically or Using Dialogs ][Printing a Document Programmatically or Using Dialogs].
...
Un singolo documento Aspose.Words può essere composto da più sezioni che specificano pagine con dimensioni, orientamento e vassoi di carta diversi. [AsposeWordsPrintDocument](../../com.aspose.words/asposewordsprintdocument/) dovrebbe essere usato come **java.awt.print.Pageable** per stampare correttamente ciascuna delle diverse dimensioni di carta, orientamento, ecc.

D'altra parte, se il documento è composto da una sola sezione, lo sviluppatore può utilizzare [AsposeWordsPrintDocument](../../com.aspose.words/asposewordsprintdocument/) come **java.awt.print.Printable** per migliorare le prestazioni di stampa.

 **Examples:** 

Mostra come monitorare l'avanzamento della stampa.
...
Mostra una classe di esempio per il monitoraggio dell'avanzamento della stampa.
...
| --- | --- |
| [AsposeWordsPrintDocument(Document document)](#AsposeWordsPrintDocument-com.aspose.words.Document) | Inizializza una nuova istanza di questa classe. |
## Metodi

| Metodo | Descrizione |
| --- | --- |
| [getColorMode()](#getColorMode) | Restituisce come vengono stampate le pagine non colorate se il dispositivo supporta la stampa a colori. |
| [getColorPagesPrinted()](#getColorPagesPrinted) | Restituisce il numero di pagine stampate a colori (cioè |
| [getNumberOfPages()](#getNumberOfPages) |  |
| [getPageFormat(int pageIndex)](#getPageFormat-int) |  |
| [getPageIndexFilter()](#getPageIndexFilter) | Restituisce il filtro di indice basato su zero usato per determinare quali pagine saltare durante la stampa. |
| [getPagesRemaining()](#getPagesRemaining) | Restituisce il numero di pagine rimanenti nel lavoro di stampa attualmente attivo. |
| [getPrintable(int pageIndex)](#getPrintable-int) |  |
| [getTotalPagesPrinted()](#getTotalPagesPrinted) | Restituisce il numero totale di pagine effettivamente stampate durante la sessione di stampa. |
| [print(Graphics graphics, PageFormat pageFormat, int pageIndex)](#print-java.awt.Graphics-java.awt.print.PageFormat-int) |  |
| [setColorMode(int value)](#setColorMode-int) | Imposta come vengono stampate le pagine non colorate se il dispositivo supporta la stampa a colori. |
| [setPageIndexFilter(IIndexFilter value)](#setPageIndexFilter-com.aspose.words.IIndexFilter) | Imposta il filtro di indice basato su zero utilizzato per determinare quali pagine saltare durante la stampa. |
...
| --- | --- | --- |
| document | [Document](../../com.aspose.words/document/) | Il documento da stampare. |
...
Restituisce come vengono stampate le pagine non colorate se il dispositivo supporta la stampa a colori.
...
**Returns:**
int - Come vengono stampate le pagine non colorate se il dispositivo supporta la stampa a colori. Il valore restituito è una delle costanti [ColorPrintMode](../../com.aspose.words/colorprintmode/).
...
Ottiene il numero di pagine stampate a colori (cioè con PageSettings\#getColor().getColor() / PageSettings\#setColor(boolean).setColor(boolean) impostato su true).

**Returns:**
int - Il numero di pagine stampate a colori (cioè.
### getNumberOfPages() {#getNumberOfPages}
```
...
Restituisce il filtro di indice basato su zero usato per determinare quali pagine saltare durante la stampa.

 **Remarks:** 

Si prega di notare che se alcune pagine vengono saltate, il numero totale effettivo di pagine da stampare non sarà noto fino al completamento del processo di stampa. Questo potrebbe influire sul monitoraggio del processo di stampa se il numero di pagine previsto inizialmente è superiore al numero effettivamente stampato.

 **Examples:** 

Mostra come filtrare le pagine utilizzando un elenco di numeri di pagina.
...
Restituisce il numero di pagine rimanenti nel lavoro di stampa attualmente attivo.

 **Remarks:** 

Questo valore viene aggiornato automaticamente man mano che le pagine vengono stampate, riflettendo l'avanzamento corrente del lavoro di stampa. Al di fuori del tempo di stampa e in caso di errori o interruzioni del lavoro di stampa, il valore potrebbe non riflettere il numero effettivo di pagine in attesa.
...
**Returns:**
int - Il numero di pagina corrente in stampa. / public int getPrintingPage() \{ return printingPage; \} /\*\* Ottiene il numero totale di pagine da stampare. Restituisce 0 quando non è in corso alcuna stampa.
### getPrintable(int pageIndex) {#getPrintable-int}
```
...
Restituisce il numero totale di pagine effettivamente stampate durante la sessione di stampa.

 **Remarks:** 

Aggiornato al termine della stampa. Restituisce 0 prima dell'inizio della stampa.
...
**Returns:**
int - true se la pagina deve essere saltata; altrimenti, false. /
### print(Graphics graphics, PageFormat pageFormat, int pageIndex) {#print-java.awt.Graphics-java.awt.print.PageFormat-int}
```
...
Imposta come vengono stampate le pagine non colorate se il dispositivo supporta la stampa a colori.
...
| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| value | int | Come vengono stampate le pagine non colorate se il dispositivo supporta la stampa a colori. Il valore deve essere una delle costanti [ColorPrintMode](../../com.aspose.words/colorprintmode/). |
...
Imposta il filtro di indice basato su zero utilizzato per determinare quali pagine saltare durante la stampa.
...
| Parametro | Tipo | Descrizione |
| --- | --- | --- |
| value | [IIndexFilter](../../com.aspose.words/iindexfilter/) | Il filtro di indice basato su zero utilizzato per determinare quali pagine saltare durante la stampa. |
