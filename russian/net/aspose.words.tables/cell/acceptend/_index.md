...
description: Откройте для себя метод Cell AcceptEnd для бесперебойного управления доступом посетителей в конечных точках ячеек, улучшения пользовательского опыта и оптимизации потока.
type: docs
weight: 140
url: /ru/net/aspose.words.tables/cell/acceptend/
---
## Cell.AcceptEnd method

Принимает посетителя для посещения конца ячейки.
