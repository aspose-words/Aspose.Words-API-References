...
| [GetEntity](../../aspose.words.layout/layoutcollector/getentity)(Node) | Возвращает непрозрачную позицию[`LayoutEnumerator`](../layoutenumerator), которая соответствует указанному узлу. Вы можете использовать возвращаемое значение в качестве аргумента для[`Current`](../layoutenumerator/current)учитывая, что документ перечисляется и документ узла одинаковы. |
...
При создании[`LayoutCollector`](../layoutcollector)и укажите объект документа[`Document`](../../aspose.words/document)для прикрепления к нему, сборщик запишет сопоставление узлов документа с объектами макета при форматировании документа в страницы.

С помощью[`GetStartPageIndex`](./getstartpageindex),Nodeи[`GetNumPagesSpanned`](./getnumpagesspanned)методы. Эти методы автоматически создают модель макета страницы документа и при необходимости обновляют поля.

Когда вам больше не нужно собирать информацию о макете, лучше всего установить[`Document`](./document)значение null чтобы избежать ненужного сбора дополнительных сопоставлений макета.
