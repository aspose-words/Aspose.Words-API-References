...
Если макет страницы документа не построен, нумератор вызывает[`UpdatePageLayout`](../../../aspose.words/document/updatepagelayout)для его создания.

Всякий раз, когда документ обновляется и создается новая модель макета страницы, для доступа к ней должен использоваться новый нумератор.
