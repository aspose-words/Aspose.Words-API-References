...
| [CustomNumberStyleFormat](../../aspose.words.lists/listlevel/customnumberstyleformat) { get; } | Получает пользовательский формат числового стиля для этого уровня списка. Например:"а, ç, ĝ,...". |
...
| [IsLegal](../../aspose.words.lists/listlevel/islegal) { get; set; } | Истина, если уровень превращает все унаследованные числа в арабские, и ложь, если он сохраняет их стиль чисел. |
...
| [TextPosition](../../aspose.words.lists/listlevel/textposition) { get; set; } | Возвращает или задает позицию (в пунктах) второй строки переноса текста для уровня списка. |
...
| static [GetEffectiveValue](../../aspose.words.lists/listlevel/geteffectivevalue)(int, NumberStyle, string) | Сообщает строковое представление объекта[`ListLevel`](../listlevel)для указанного индекса элемента списка. Параметры определяют[`NumberStyle`](../../aspose.words/numberstyle)и необязательную строку формата , используемую, когдаCustomуказано. |

### Примечания

Вы не создаете объекты этого класса. Объекты уровня списка создаются автоматически при создании списка. Вы получаете доступ к объектам[`ListLevel`](../listlevel)через коллекцию [`ListLevelCollection`](../listlevelcollection).

Используйте свойства[`ListLevel`](../listlevel)для указания форматирования списка для отдельных уровни списка.
