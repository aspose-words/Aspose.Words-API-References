...
description: Откройте для себя свойство ListLevel NumberFormat, которое позволяет легко настраивать и устанавливать уникальные форматы чисел для списков, повышая ясность и стиль вашего документа.
type: docs
weight: 70
...
Например, строка "\x0000.\x0001)" сгенерирует список label , который выглядит примерно так: "1.5)". Число "1" — это текущий номер from 1-го уровня списка, число "5" — это текущий номер из 2-го уровня списка.

Значение Null не допускается, но пустая строка, означающая отсутствие числа, не является допустимой.
