...
| [List](../../aspose.words.lists/listformat/list) { get; set; } | Получает или задает список, членом которого является данный абзац. |
...
Абзац в документе Microsoft Word можно маркировать или нумеровать. Когда абзац промаркирован или пронумерован, говорят, что форматирование списка применяется к абзацу.

Вы не создаете объекты класса[`ListFormat`](../listformat)напрямую. Вы обращаетесь к[`ListFormat`](../listformat)как к свойству другого объекта, который может иметь связанное с ним форматирование списка. На данный момент объекты, которые могут иметь форматирование списка:[`Paragraph`](../../aspose.words/paragraph), [`Style`](../../aspose.words/style)и[`DocumentBuilder`](../../aspose.words/documentbuilder).
...
Само форматирование списка хранится внутри объекта[`List`](../list) , который хранится отдельно от абзацев. Объекты списка хранятся внутри коллекции[`ListCollection`](../listcollection). Существует единственная коллекция [`ListCollection`](../listcollection)для[`Document`](../../aspose.words/document).

Абзацы физически не принадлежат списку. Абзацы просто ссылаются на конкретный объект списка через[`List`](./list)свойство и определенный уровень в list через свойство[`ListLevelNumber`](./listlevelnumber). Установив эти два свойства, вы определяете, какие маркеры и нумерация применяются к абзацу.
