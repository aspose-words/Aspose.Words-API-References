...
description: Metered SetMeteredKey метод. Устанавливает лимитированный открытый и закрытый ключ. Если вы приобретаете лимитированную лицензию при запуске приложения этот API должен вызываться обычно этого достаточно. Однако если всегда не удается загрузить данные о потреблении и превышает 24 часа лицензия будет переведена в оценочный статус  чтобы избежать такого случая вам следует регулярно проверять статус лицензии. Если это оценочный статус вызовите этот API еще раз на С#.
type: docs
weight: 20
