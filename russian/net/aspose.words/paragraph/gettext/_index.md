...
description: Получает текст этого абзаца включая символ конца абзаца.
type: docs
weight: 260
...
Текст всех дочерних узлов объединяется, и символ конца абзаца добавляется следующим образом:

* Если абзац является последним абзацем[`Body`](../../body), затем [`ControlChar.SectionBreak`](../../controlchar/sectionbreak) (\x000c).
* Если абзац является последним абзацем в[`Cell`](../../../aspose.words.tables/cell), то [`ControlChar.Cell`](../../controlchar/cell) (\x0007) добавляется.
