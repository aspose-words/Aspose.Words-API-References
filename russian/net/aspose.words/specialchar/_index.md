...
Документ Microsoft Word может содержать ряд специальных символов которые представляют поля, поля форм, фигуры, объекты OLE, сноски и т. д. Список специальных символов см. в[`ControlChar`](../controlchar).

**SpecialChar** является встроенным узлом и может быть только потомком **Абзац** .

**SpecialChar** char используется как базовый класс для более конкретных классов представляют собой специальные символы, к которым Aspose.Words предоставляет программный доступ. Класс **SpecialChar** сам по себе также используется для представления специального символа, для которого Aspose.Words не предоставляет подробной информации. программный доступ.
