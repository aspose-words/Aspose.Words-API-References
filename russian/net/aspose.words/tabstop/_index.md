...
description: Представляет одну пользовательскую позицию табуляции. Объект TabStop является членом коллекции TabStopCollection./tabstopcollection.
type: docs
weight: 5850
...
Представляет одну пользовательскую позицию табуляции. Объект **TabStop** является членом коллекции [`TabStopCollection`](../tabstopcollection).
...
| [Position](../../aspose.words/tabstop/position) { get; } | Получает позицию табулятора в пунктах. |
...
Обычно табуляция определяет позицию, в которой существует табуляция. Но поскольку позиции табуляции могут быть унаследованы от родительских стилей, может потребоваться, чтобы дочерний объект явно определял отсутствие позиции табуляции в данной позиции. Чтобы очистить унаследованную позицию табуляции в данной позиции, создайте объект **TabStop** и установите [`Alignment`](./alignment)to` TabAlignment.Clear` .
