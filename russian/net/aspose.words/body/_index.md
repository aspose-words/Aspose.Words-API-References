...
| Имя | Описание |
| --- | --- |
| [Body](body)(DocumentBase) | Инициализирует новый экземпляр класса **Body** . |
...
| [EnsureMinimum](../../aspose.words/body/ensureminimum)() | Если последний потомок не является абзацем, создает и добавляет один пустой абзац. |
...
**Тело** может содержать **Параграф** и **Таблица** дочерние узлы.

**Body** является узлом уровня раздела и может быть только потомком **Раздел** . Может быть только один **Тело** в разделе .

Минимальный допустимый **Тело** должен содержать хотя бы один **Абзац** .
