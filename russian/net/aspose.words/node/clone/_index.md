...
| --- | --- | --- |
| isCloneChildren | Boolean | Значение true, чтобы рекурсивно клонировать поддерево под указанным узлом; false, чтобы клонировать только сам узел. |
...
Этот метод всегда выполняет глубокое копирование узла. Параметр isCloneChildren указывает, следует ли копировать также все дочерние узлы.
