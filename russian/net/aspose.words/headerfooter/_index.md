...
| [IsHeader](../../aspose.words/headerfooter/isheader) { get; } | Истинно, если этот объект **HeaderFooter** является заголовком. |
...
**HeaderFooter** может содержать **Параграф** и **Таблица** дочерние узлы.

**HeaderFooter** является узлом уровня раздела и может быть только потомком **Раздел** . Может быть только один **HeaderFooter** или каждый[`HeaderFooterType`](./headerfootertype)в **Раздел** .

Если **Раздел** не имеет **HeaderFooter** определенного типа или  **HeaderFooter** не имеет дочерних узлов, этот верхний/нижний колонтитул считается связанным с верхний/нижний колонтитул того же типа, что и предыдущий раздел в Microsoft Word.

Когда **HeaderFooter** содержит хотя бы один **Параграф** , он больше не считается связанным с предыдущим в Microsoft Word.
