---
title: Enum TabAlignment
second_title: Справочник по API Aspose.Words для .NET
description: Aspose.Words.TabAlignment перечисление. Определяет выравнивание/тип табуляции.
type: docs
weight: 6180
url: /ru/net/aspose.words/tabalignment/
---
## TabAlignment enumeration

Определяет выравнивание/тип табуляции.

```csharp
public enum TabAlignment
```

### Ценности

| Имя | Ценность | Описание |
| --- | --- | --- |
| Left | `0` | Выравнивает текст после табуляции по левому краю. |
| Center | `1` | Центрирует текст вокруг позиции табуляции. |
| Right | `2` | Выравнивает текст по правому краю табуляции. |
| Decimal | `3` | Выравнивает текст по десятичной точке. |
| Bar | `4` | Рисует вертикальную полосу в позиции табуляции. |
| List | `6` | Табуляция является разделителем между номером/маркером и текстом в элементе списка. |
| Clear | `7` | Удаляет любую позицию табуляции в этой позиции. |

### Смотрите также

* пространство имен [Aspose.Words](../../aspose.words/)
* сборка [Aspose.Words](../../)


