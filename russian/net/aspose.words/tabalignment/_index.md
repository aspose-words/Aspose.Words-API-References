---
title: Enum TabAlignment
second_title: Справочник по API Aspose.Words для .NET
description: Aspose.Words.TabAlignment перечисление. Определяет выравнивание/тип позиции табуляции.
type: docs
weight: 5880
url: /ru/net/aspose.words/tabalignment/
---
## TabAlignment enumeration

Определяет выравнивание/тип позиции табуляции.

```csharp
public enum TabAlignment
```

### Ценности

| Имя | Ценность | Описание |
| --- | --- | --- |
| Left | `0` | Выравнивает текст после позиции табуляции по левому краю. |
| Center | `1` | Центрирует текст вокруг позиции табуляции. |
| Right | `2` | Выравнивает текст по правому краю на позиции табуляции. |
| Decimal | `3` | Выравнивает текст по десятичной точке. |
| Bar | `4` | Рисует вертикальную черту в позиции табуляции. |
| List | `6` | Вкладка является разделителем между числом/маркером и текстом в элементе списка. |
| Clear | `7` | Очищает любую позицию табуляции в этой позиции. |

### Смотрите также

* пространство имен [Aspose.Words](../../aspose.words/)
* сборка [Aspose.Words](../../)


