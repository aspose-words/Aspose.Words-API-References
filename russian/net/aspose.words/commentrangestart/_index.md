...
description: Aspose.Words.CommentRangeStart сорт. Обозначает начало области текста с которой связан комментарий.
type: docs
weight: 250
...
| Имя | Описание |
| --- | --- |
| [CommentRangeStart](commentrangestart/)(DocumentBase, int) | Инициализирует новый экземпляр этого класса. |
...
| [Id](../../aspose.words/commentrangestart/id/) { get; set; } | Указывает идентификатор комментария, к которому привязан этот регион. |
...
| Имя | Описание |
| --- | --- |
| override [Accept](../../aspose.words/commentrangestart/accept/)(DocumentVisitor) | Принимает посетителя. |
...
Чтобы создать комментарий, привязанный к области текста, необходимо создать[`Comment`](../comment/) and затем создайте`CommentRangeStart` а также[`CommentRangeEnd`](../commentrangeend/)и установите их идентификаторы одинаковыми[`Id`](../comment/id/) ценность.

`CommentRangeStart` является узлом встроенного уровня и может быть только потомком[`Paragraph`](../paragraph/).
