...
description: EditableRange SingleUser свойство. Возвращает или устанавливает отдельного пользователя для редактируемого диапазона на С#.
type: docs
weight: 50
...
Возвращает или устанавливает отдельного пользователя для редактируемого диапазона.
...
Этот редактор можно сохранить в одной из следующих форм:

ДОМЕН\Имя пользователя — для пользователей, чей доступ должен быть аутентифицирован с использованием учетных данных домена текущего пользователя.

user@domain.com — для пользователей, доступ которых должен быть подтвержден с использованием адреса электронной почты пользователя в качестве учетных данных.
...
Один пользователь и группа редакторов не могут быть установлены одновременно для определенного редактируемого диапазона. , если один из них установлен, другой будет неактивен.
