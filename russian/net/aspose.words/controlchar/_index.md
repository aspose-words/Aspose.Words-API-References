...
description: Часто встречающиеся в документах управляющие символы.
type: docs
weight: 340
...
Часто встречающиеся в документах управляющие символы.
...
| static readonly [Cr](../../aspose.words/controlchar/cr) | Символ возврата каретки:"\x000d" или "\r". То же, что[`ParagraphBreak`](./paragraphbreak). |
| static readonly [CrLf](../../aspose.words/controlchar/crlf) | Возврат каретки с последующим символом перевода строки:"\x000d\x000a" или "\r\n". Не используется как таковой в документах Microsoft Word, но обычно используется в текстовых файлах для разрывов абзаца. |
...
| const [FieldSeparatorChar](../../aspose.words/controlchar/fieldseparatorchar) | Знак-разделитель полей отделяет код поля от значения поля. Необязательный в некоторых полях. Значение:(знак)20. |
| const [FieldStartChar](../../aspose.words/controlchar/fieldstartchar) | Символ начала поля MS Word:(char)19. |
...
| const [NonBreakingHyphenChar](../../aspose.words/controlchar/nonbreakinghyphenchar) | Неразрывный дефис в Microsoft Word равен (char)30. |
...
| const [SpaceChar](../../aspose.words/controlchar/spacechar) | Пробел:(char)32. |
...
Предоставляет как символьные, так и строковые версии одних и тех же констант. Например: string ControlChar.LineBreak и char ControlChar.LineBreakChar имеют одно и то же значение.
