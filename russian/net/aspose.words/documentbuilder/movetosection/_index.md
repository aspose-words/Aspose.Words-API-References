...
| --- | --- | --- |
| sectionIndex | Int32 | Индекс раздела для перехода. |

### Примечания

Когда sectionIndex больше или равен 0, он указывает индекс из начало документа с 0, являющимся первым разделом. Когда sectionIndex меньше 0, указывается индекс с конца документа, где -1 является последним разделом.

Курсор перемещается на первый абзац в **Body** указанного раздела.
