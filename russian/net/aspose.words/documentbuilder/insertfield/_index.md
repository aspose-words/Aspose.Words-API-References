...
Этот метод вставляет поле в документ. Aspose.Words может обновлять поля большинства типов, но не всех. Дополнительные сведения см. в перегрузке [`InsertField`](../insertfield).
...
| Параметр | Тип | Описание |
| --- | --- | --- |
| fieldCode | String | Код поля для вставки (без фигурные скобки). |
...
Этот метод вставляет поле в документ и немедленно обновляет результат поля. Aspose.Words может обновлять поля большинства типов, но не всех. Дополнительные сведения см. в перегрузке [`InsertField`](../insertfield).
...
Поля в документах Microsoft Word состоят из кода поля и результата поля. Код поля подобен формуле, а результат поля подобен значению, которое выдает формула. Код поля также может содержать переключатели полей , которые являются своего рода дополнительными инструкциями для выполнения определенного действия.
...
Чтобы создать поле, необходимо указать тип поля, код поля и значение поля "заполнитель". Если вы не уверены в синтаксисе определенного кода поля, сначала создайте поле в Microsoft Word и переключитесь, чтобы просмотреть его код поля.

Aspose.Words может вычислять результаты поля для большинства типов полей, но этот метод не обновляет результат поля автоматически. Поскольку результат поля не вычисляется автоматически, ожидается, что вы передадите некоторое строковое значение (или даже пустую строку), которое будет вставлено в результат поля. Это значение останется в поле результата в качестве заполнителя, пока поле не будет обновлено. Чтобы обновить результат поля, вы можете вызвать[`Update`](../../../aspose.words.fields/field/update)для возвращаемого вам объекта поля или[`UpdateFields`](../../document/updatefields)для обновления полей во всем документе.
