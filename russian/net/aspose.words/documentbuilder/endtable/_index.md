...
description: С легкостью завершайте таблицы документов с помощью метода EndTable в DocumentBuilder, гарантируя бесшовное форматирование и профессиональную презентацию.
type: docs
weight: 250
...
Узел таблицы, который был только что завершен.

## Примечания

Этот метод следует вызывать только один раз после[`EndRow`](../endrow/) был вызван. Когда был вызван, `EndTable` перемещает курсор из текущей ячейки в положение сразу после таблицы.
