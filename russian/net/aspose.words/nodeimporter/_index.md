...
Однако, когда вам нужно несколько раз импортировать узлы из одного документа в другой, лучше использовать`NodeImporter` учебный класс.`NodeImporter` Класс позволяет минимизировать количество стилей и списков, создаваемых в целевом документе.
...
`NodeImporter`class подобен контексту, который содержит "таблицы перевода" во время импорта. Он корректно переводит стили и списки в исходный и конечный документы.
