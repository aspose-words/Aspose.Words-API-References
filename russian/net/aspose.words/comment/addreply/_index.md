...
description: Добавляет ответ на этот комментарий.
type: docs
weight: 120
...
Созданный узел[`Comment`](../../comment)для ответа.

### Примечания

В связи с существующими ограничениями MS Office в документе допускается только 1 уровень ответов. Исключение типаInvalidOperationExceptionбудет вызвано, если этот метод вызывается для существующего комментария Reply.
