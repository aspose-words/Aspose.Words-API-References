---
title: Aspose.Words.Drawing.Ole
linktitle: Aspose.Words.Drawing.Ole
articleTitle: Aspose.Words.Drawing.Ole
second_title: Aspose.Words для .NET
description: Содержит классы для работы с элементами управления OLE/ActiveX и формами в документах Microsoft Word на С#.
type: docs
weight: 70
url: /ru/net/aspose.words.drawing.ole/
---
Содержит классы для работы с элементами управления OLE/ActiveX и формами в документах Microsoft Word.

## Классы

| Учебный класс | Описание |
| --- | --- |
| [Forms2OleControl](./forms2olecontrol/) | Представляет элемент управления OLE Microsoft Forms 2.0. |
| [Forms2OleControlCollection](./forms2olecontrolcollection/) | Представляет коллекцию[`Forms2OleControl`](../aspose.words.drawing.ole/forms2olecontrol/) объекты. |
| [OleControl](./olecontrol/) | Представляет элемент управления OLE ActiveX. |
## перечисление

| перечисление | Описание |
| --- | --- |
| [Forms2OleControlType](./forms2olecontroltype/) | Перечисляет типы элементов управления Forms 2.0. |
