...
description: Узнайте, как использовать свойство запроса MailMergeSettings для эффективного импорта записей из внешних источников данных для бесперебойных операций слияния почты.
type: docs
weight: 160
...
Содержит строку языка структурированных запросов, которая должна быть запущена для указанного внешнего источника данных, чтобы вернуть набор записей, которые должны быть импортированы в документ при выполнении операции слияния почты. Значением по умолчанию является пустая строка.
