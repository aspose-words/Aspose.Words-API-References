---
title: ChartLegendEntry
second_title: Справочник по API Aspose.Words для .NET
description: Представляет запись легенды диаграммы.
type: docs
weight: 690
url: /ru/net/aspose.words.drawing.charts/chartlegendentry/
---
## ChartLegendEntry class

Представляет запись легенды диаграммы.

```csharp
public class ChartLegendEntry
```

## Характеристики

| Имя | Описание |
| --- | --- |
| [Font](../../aspose.words.drawing.charts/chartlegendentry/font) { get; } | Предоставляет доступ к форматированию шрифта этой записи легенды. |
| [IsHidden](../../aspose.words.drawing.charts/chartlegendentry/ishidden) { get; set; } | Получает или задает значение, указывающее, скрыта ли эта запись в легенде диаграммы. Значение по умолчанию: **false** . |

### Примечания

Запись легенды соответствует определенной серии диаграммы или линии тренда.

Текст записи - название серии или линии тренда. Текст нельзя изменить.

### Смотрите также

* пространство имен [Aspose.Words.Drawing.Charts](../../aspose.words.drawing.charts)
* сборка [Aspose.Words](../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Words.dll -->
