...
description: Позволяет указать должен ли размер пузырька отображаться для меток данных всего ряда. Применяется только к пузырьковым диаграммам. Значение по умолчанию ЛОЖЬ .
type: docs
weight: 50
