...
description: Откройте для себя Aspose.Words.Saving.XlsxSaveOptions для улучшения преобразования документов в Xlsx. Разблокируйте расширенные параметры сохранения для оптимальных результатов!
type: docs
weight: 6520
...
Чтобы узнать больше, посетите[Укажите Сохранить параметры](https://docs.aspose.com/words/net/specify-save-options/) документальная статья.
...
| [DateTimeParsingMode](../../aspose.words.saving/xlsxsaveoptions/datetimeparsingmode/) { get; set; } | Возвращает или задает режим, который определяет, как анализируется текст документа для определения значений даты и времени. Значение по умолчанию:UseCurrentLocale . |
...
| override [SaveFormat](../../aspose.words.saving/xlsxsaveoptions/saveformat/) { get; set; } | Указывает формат, в котором будет сохранен документ, если используется этот объект параметров сохранения. Может быть толькоXlsx . |
| [SectionMode](../../aspose.words.saving/xlsxsaveoptions/sectionmode/) { get; set; } | Возвращает или задает способ обработки разделов при сохранении в выходной документ XLSX. Значение по умолчанию:MultipleWorksheets . |
