...
description: Получает или задает значение определяющее использовать ли средство визуализации метафайлов GDI или Aspose.Words при сохранении в EMF.
type: docs
weight: 180
...
Если установлено` true` Используется средство визуализации метафайлов GDI+. Т.е. контент записывается в объект GDI+ Graphics и сохраняется в метафайл.

Если установлено значение` false` Используется средство визуализации метафайлов Aspose.Words. Т.е. контент записывается прямо в формат метафайла с Aspose.Words.

Действует только при сохранении в EMF.

GDI+ сохранение работает только на .NET.
