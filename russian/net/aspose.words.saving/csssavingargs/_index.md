...
description: Предоставляет данные для событияCssSaving./icsssavingcallback/csssaving.
type: docs
weight: 4570
...
Предоставляет данные для события[`CssSaving`](../icsssavingcallback/csssaving).
...
| [IsExportNeeded](../../aspose.words.saving/csssavingargs/isexportneeded) { get; set; } | Позволяет указать, будет ли CSS экспортироваться в файл и внедряться в HTML-документ. Значение по умолчанию:` true` . Когда это свойство равно` false` , информация CSS не будет сохранена в файле CSS и не будет встроена в документ HTML. |
...
По умолчанию, когда Aspose.Words сохраняет документ в HTML, он сохраняет встроенную информацию CSS (как значение атрибута **style** для каждого элемента).

[`CssSavingArgs`](../csssavingargs)позволяет сохранять информацию CSS в файл, предоставляя собственный потоковый объект.

Чтобы сохранить CSS в поток, используйте свойство[`CssStream`](./cssstream).

Чтобы отключить сохранение CSS в файл и встраивание в HTML-документ, используйте свойство[`IsExportNeeded`](./isexportneeded).
