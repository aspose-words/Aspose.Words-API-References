---
title: ForcePageBreaks
second_title: Справочник по API Aspose.Words для .NET
description: Позволяет указать должны ли сохраняться разрывы страниц при экспорте.
type: docs
weight: 30
url: /ru/net/aspose.words.saving/txtsaveoptionsbase/forcepagebreaks/
---
## TxtSaveOptionsBase.ForcePageBreaks property

Позволяет указать, должны ли сохраняться разрывы страниц при экспорте.

Значение по умолчанию: **false** .

```csharp
public bool ForcePageBreaks { get; set; }
```

### Примечания

Свойство влияет только на разрывы страниц, явно вставленные в документ. Это не связано с разрывами страниц, которые MS Word автоматически вставляет в конце каждой страницы.

### Примеры

Показывает, как указать, сохранять ли разрывы страниц при экспорте документа в открытый текст.

```csharp
Document doc = new Document();
DocumentBuilder builder = new DocumentBuilder(doc);

builder.Writeln("Page 1");
builder.InsertBreak(BreakType.PageBreak);
builder.Writeln("Page 2");
builder.InsertBreak(BreakType.PageBreak);
builder.Writeln("Page 3");

 // Создадим объект "TxtSaveOptions", который мы можем передать в "Сохранить" документа
 // метод изменения способа сохранения документа в виде открытого текста.
TxtSaveOptions saveOptions = new TxtSaveOptions();

 // Объекты Aspose.Words "Document" имеют разрывы страниц, как документы Microsoft Word.
// Форматы сохранения, такие как ".txt", представляют собой один непрерывный текст без разрывов страниц.
 // Установите для свойства "ForcePageBreaks" значение "true", чтобы сохранить все разрывы страниц в виде символов '\f'.
 // Установите для свойства "ForcePageBreaks" значение "false", чтобы отменить все разрывы страниц.
saveOptions.ForcePageBreaks = forcePageBreaks;

doc.Save(ArtifactsDir + "TxtSaveOptions.PageBreaks.txt", saveOptions);

 // Если мы загружаем текстовый документ с разрывами страниц, 
 // объект "Документ" будет использовать их для разделения тела на страницы.
doc = new Document(ArtifactsDir + "TxtSaveOptions.PageBreaks.txt");

Assert.AreEqual(forcePageBreaks ? 3 : 1, doc.PageCount);
```

### Смотрите также

* class [TxtSaveOptionsBase](../../txtsaveoptionsbase)
* namespace [Aspose.Words.Saving](../../txtsaveoptionsbase)
* assembly [Aspose.Words](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Words.dll -->
