...
description: Узнайте, как свойство KeepDocumentPartStreamOpen в DocumentPartSavingArgs позволяет Aspose.Words эффективно управлять потоками частей документа для оптимальной производительности.
type: docs
weight: 40
...
Указывает, должен ли Aspose.Words держать поток открытым или закрыть его после сохранения части документа.
...
По умолчанию`ЛОЖЬ` и Aspose.Words закроет предоставленный вами поток в[`DocumentPartStream`](../documentpartstream/) свойство после записи в него части документа. Укажите`истинный` чтобы держать поток открытым. Обратите внимание, что основной выходной поток , предоставленный в вызове[`Save`](../../../aspose.words/document/save/)или [`Save`](../../../aspose.words/document/save/) никогда не будет закрыт Aspose.Words , даже если`KeepDocumentPartStreamOpen` установлен на`ЛОЖЬ`.
