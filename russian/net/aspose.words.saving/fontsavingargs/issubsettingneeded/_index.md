...
description: Позволяет указать будет ли текущий шрифт подмножаться перед экспортом в качестве ресурса шрифта.
type: docs
weight: 70
...
Шрифты можно экспортировать как полные исходные файлы шрифтов или подмножества, включающие только символы которые используются в документе. Подмножество позволяет уменьшить результирующий размер ресурса шрифта.

По умолчанию Aspose.Words решает, выполнять ли подмножество или нет, сравнивая исходный размер файла шрифта с размером, указанным в[`FontResourcesSubsettingSizeThreshold`](../../htmlsaveoptions/fontresourcessubsettingsizethreshold). Вы можете переопределить это поведение для отдельных шрифтов, задав свойство`IsSubsettingNeeded`.
