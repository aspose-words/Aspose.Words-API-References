...
Это абстрактный класс. Вы можете создать два производных класса [`Shape`](../shape)и[`GroupShape`](../groupshape).

Фигура — это узел в дереве документа.

Если фигура является потомком объекта[`Paragraph`](../../aspose.words/paragraph), то фигура считается "верхней". -уровень". Фигуры верхнего уровня измеряются и располагаются в точках.

Фигура также может быть дочерней для объекта[`GroupShape`](../groupshape), когда несколько фигур сгруппированы. Дочерние фигуры групповой формы располагаются в координатном пространстве и единицах , определенных[`CoordSize`](./coordsize)и[`CoordOrigin`](./coordorigin)свойства родителя фигура группы.

Фигура может быть встроена в текст или плавать. Способ позиционирования управляется с помощью свойства[`WrapType`](./wraptype).

Когда фигура плавает, она позиционируется относительно чего-либо (например, текущего абзаца, поля или страницы). Относительное расположение фигуры задается с помощью [`RelativeHorizontalPosition`](./relativehorizontalposition)и[`RelativeVerticalPosition`](./relativeverticalposition)свойства.

Плавающая фигура может быть позиционирована явно с помощью[`Left`](./left)и[`Top`](./top) свойства или выровнены относительно какого-либо другого объекта с помощью[`HorizontalAlignment`](./horizontalalignment) и[`VerticalAlignment`](./verticalalignment)свойства.
