...
description: Определяет порядок отображения перекрывающихся фигур.
type: docs
weight: 540
...
Порядок наложения фигур независим для фигур в заголовке и в основном тексте документа.

Порядок отображения дочерних фигур в фигуре группы определяется их порядком внутри фигуры группы.

### Примеры

Показывает, как управлять порядком формы.
