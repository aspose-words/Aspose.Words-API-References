...
A[`GroupShape`](../groupshape)is составной узел и может иметь узлы[`Shape`](../shape)и [`GroupShape`](../groupshape)как узлы дети.

Каждый[`GroupShape`](../groupshape)определяет новую систему координат для своих дочерних фигур. Система координат определяется с помощью[`CoordSize`](../shapebase/coordsize)и [`CoordOrigin`](../shapebase/coordorigin)свойства.
