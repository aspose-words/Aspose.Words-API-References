...
description: Определяет будет ли обратный порядок расположения строк.
type: docs
weight: 80
...
Определяет, будет ли обратный порядок расположения строк.
...
Если **true** , порядок расположения строк меняется на обратный. Этот атрибут используется для вертикального расположения текста.
