...
description: Представляет корневой элемент документа глоссария в документе Word. Глоссарий  это хранилище для автотекста записей автозамены и стандартных блоков.
type: docs
weight: 170
...
Представляет корневой элемент документа глоссария в документе Word. Глоссарий — это хранилище для автотекста, записей автозамены и стандартных блоков.
...
| override [NodeType](../../aspose.words.buildingblocks/glossarydocument/nodetype) { get; } | Возвращает значениеGlossaryDocument. |
...
| [ImportNode](../../aspose.words/documentbase/importnode)(Node, bool, ImportFormatMode) | Импорт узла из другого документа в текущий документ с возможностью управления форматированием. |
...
Некоторые документы, обычно шаблоны, могут содержать записи автотекста, автозамены и/или Строительные блоки (также известные как записи документа глоссария , части документа или строительные блоки ).

Чтобы получить доступ к строительным блокам, вам необходимо загрузить документ в[`Document`](../../aspose.words/document) объект. Стандартные блоки будут доступны через свойство[`GlossaryDocument`](../../aspose.words/document/glossarydocument).

[`GlossaryDocument`](../glossarydocument)может содержать любое количество[`BuildingBlock`](../buildingblock)объекты. Каждый[`BuildingBlock`](../buildingblock)представляет одну часть документа.

Соответствует глоссарию **Документ** и **docParts**элементы в формате OOXML.
