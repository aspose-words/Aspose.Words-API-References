...
Соответствующий строительный блок или нуль, если совпадение не найдено.

### Примечания

Это удобный метод, который перебирает все строительные блоки в эту коллекцию и возвращает первый стандартный блок, соответствующий указанной галерее, категории и имени.

Microsoft Word организует стандартные блоки в галереи. Галереи предопределены с помощью перечисления[`BuildingBlockGallery`](../../buildingblockgallery). В каждой галерее стандартные блоки могут быть организованы в одну или несколько категорий. Имя категории представляет собой строку. Каждый строительный блок имеет имя. Уникальность имени стандартного блока не гарантируется.
