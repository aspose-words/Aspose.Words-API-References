...
description: Представляет запись документа глоссария такую как стандартный блок автотекст или запись автозамены.
type: docs
weight: 120
...
| [Guid](../../aspose.words.buildingblocks/buildingblock/guid) { get; set; } | Получает или задает идентификатор (128-разрядный GUID), однозначно идентифицирующий этот стандартный блок. |
...
| override [NodeType](../../aspose.words.buildingblocks/buildingblock/nodetype) { get; } | Возвращает значениеBuildingBlock. |
...
| [Type](../../aspose.words.buildingblocks/buildingblock/type) { get; set; } | Задает тип стандартного блока. |
...
[`BuildingBlock`](../buildingblock)может быть только потомком[`GlossaryDocument`](../glossarydocument).

Вы можете создавать новые стандартные блоки и вставлять их в глоссарий. Вы можете изменить или удалить существующие строительные блоки. Вы можете копировать или перемещать строительные блоки между документами. Вы можете вставить содержимое стандартного блока в документ.

Соответствует **docPart** , **docPartPr** и **docPartBody** элементы в OOXML.
