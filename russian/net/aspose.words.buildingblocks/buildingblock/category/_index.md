...
description: Задает категоризацию второго уровня для стандартного блока.
type: docs
weight: 30
...
Строительные блоки в пользовательском интерфейсе Microsoft Word организованы в Галереи. Каждый[`Gallery`](../gallery)может иметь несколько категорий. Каждый блок в пределах a`Category`имеет[`Name`](../name).

Не может быть` null` и не может быть пустой строкой.

Соответствует элементу **docPartPr.category.name** в OOXML.
