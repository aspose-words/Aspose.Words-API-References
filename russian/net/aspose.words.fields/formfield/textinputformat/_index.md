...
description: Откройте для себя свойство FormField TextInputFormat, которое позволяет легко настраивать форматирование текста для ваших форм, улучшая пользовательский интерфейс и сбор данных.
type: docs
weight: 200
...
Возвращает или задает форматирование текста для поля текстовой формы.
...
Если поле текстовой формы содержит обычный текст, то допустимыми строками формата являются "", "ВЕРХНИЙ РЕГИСТР", "СТРОЧНЫЙ РЕГИСТР", "ПЕРВЫЙ ЗАГЛАВНЫЙ" и "ЗАГЛАВНЫЙ РЕГИСТР". Строки нечувствительны к регистру.

Если поле текстовой формы содержит число или значение даты/времени, то строки формата valid представляют собой строки формата числа или даты и времени.
