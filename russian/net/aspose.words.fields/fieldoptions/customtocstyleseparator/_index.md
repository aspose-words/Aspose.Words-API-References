...
description: Откройте для себя свойство FieldOptions CustomTocStyleSeparator, чтобы легко настроить разделитель стилей полей FieldToc для улучшенной навигации по документу.
type: docs
weight: 60
...
По умолчанию пользовательские стили определяются переключателем \t в[`FieldToc`](../../fieldtoc/) поля разделены разделителем, взятым из текущей культуры. Это свойство переопределяет это поведение, указывая определенный пользователем разделитель.
