...
| [FormatAddressOnCountryOrRegion](../../aspose.words.fields/fieldaddressblock/formataddressoncountryorregion) { get; set; } | Получает или задает форматирование адреса в соответствии со страной/регионом получателя как определено POST*CODE (Всемирный почтовый союз 2006 г.) ). |
...
Представляет блок адреса. Блок адреса представляет собой блок текста, определяющий информацию , соответствующую почтовому почтовому адресу, в порядке, требуемом страной назначения.
