...
Полное поле в документе Microsoft Word представляет собой сложную структуру, состоящую из символ начала поля, код поля, символ-разделитель полей, результат поля и символ конца поля. Некоторые поля имеют только начало поля, код поля и конец поля.
