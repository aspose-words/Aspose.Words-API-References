...
**Row** может быть только ребенком**Table**.

**Row** может содержать один или несколько**Cell** узлы.

 В минимально допустимой строке должен быть хотя бы один**Cell**.
## Конструкторы

| Конструктор | Описание |
| --- | --- |
| [Row(DocumentBase doc)](#Row-com.aspose.words.DocumentBase-) |  Инициализирует новый экземпляр**Row** учебный класс. |
## Методы
...
| [ensureMinimum()](#ensureMinimum--) |  Если**Row** не имеет ячеек, создает и добавляет одну**Cell**. |
...
| [getCells()](#getCells--) |  Обеспечивает типизированный доступ к**Cell** дочерние узлы строки. |
...
| [getFirstCell()](#getFirstCell--) |  Возвращает первый**Cell** в ряду. |
| [getFirstChild()](#getFirstChild--) | Получает первый дочерний элемент узла. |
| [getLastCell()](#getLastCell--) |  Возвращает последний**Cell** в ряду. |
...
| [getNodeType()](#getNodeType--) |  Возвращает**NodeType.Row**. |
...
| [getRowFormat()](#getRowFormat--) | Предоставляет доступ к свойствам форматирования строки. |
...
| [isFirstRow()](#isFirstRow--) | Истинно, если это первая строка в таблице; ложно в противном случае. |
| [isLastRow()](#isLastRow--) | Истинно, если это последняя строка в таблице; ложно в противном случае. |
...
```
 ...
 Инициализирует новый экземпляр**Row** учебный класс.

 Когда**Row** создан, он принадлежит указанному документу, но еще не является его частью и**ParentNode** нулевой.

 Чтобы добавить**Row** к документу используйте InsertAfter или InsertBefore в таблице, в которую вы хотите вставить строку.
...
**Возвращает:**
boolean - Истинно, если были посещены все узлы; false, если DocumentVisitor остановил операцию перед посещением всех узлов. Вызывает DocumentVisitor.VisitRowStart, затем вызывает Accept для всех дочерних узлов раздела и в конце вызывает DocumentVisitor.VisitRowEnd.
...
```
 ...
 Если**Row** не имеет ячеек, создает и добавляет одну**Cell**.
...
```
 ...
 Обеспечивает типизированный доступ к**Cell** дочерние узлы строки.

**Возвращает:**
[CellCollection](../../com.aspose.words/cellcollection) - соответствующий[CellCollection](../../com.aspose.words/cellcollection) ценность.
### getChild(int nodeType, int index, boolean isDeep) {#getChild-int-int-boolean-}
```
...
```
 ...
 Возвращает первый**Cell** в ряду.

**Возвращает:**
[Cell](../../com.aspose.words/cell) - Первый**Cell** в ряду.
...
```
 ...
 Возвращает последний**Cell** в ряду.

**Возвращает:**
[Cell](../../com.aspose.words/cell) - Последний**Cell** в ряду.
...
```
 ...
 Возвращает**NodeType.Row**.

**Возвращает:**
 инт -**NodeType.Row** . Возвращаемое значение является одним из[NodeType](../../com.aspose.words/nodetype) константы.
...
Возвращает непосредственную родительскую таблицу строки. Эквивалент (Таблица)FirstNonMarkupParentNode .

**Возвращает:**
[Table](../../com.aspose.words/table) - Непосредственная родительская таблица строки.
...
**Возвращает:**
[RowFormat](../../com.aspose.words/rowformat) - соответствующий[RowFormat](../../com.aspose.words/rowformat) ценность.
### getText() {#getText--}
```
...
Получает текст всех ячеек в этой строке, включая символ конца строки.
 ...
 Возвращает объединенный текст всех дочерних узлов с символом конца строки[ControlChar.CELL](../../com.aspose.words/controlchar\#CELL) добавлено в конце.
...
Истинно, если это первая строка в таблице; ложно в противном случае.
...
Истинно, если это последняя строка в таблице; ложно в противном случае.
