...
Извлекает имя текущего документа из места его хранения.
 ...
 В текущей реализации используется[Document.getOriginalFileName()](../../com.aspose.words/document\#getOriginalFileName--) свойство для получения имени файла. Если документ был загружен из потока или создан пустым, используется имя файла, в который сохраняется (если известно).
## Методы
...
| [getIncludeFullPath()](#getIncludeFullPath--) | Определяет, следует ли включать полное имя пути к файлу. |
...
Определяет, следует ли включать полное имя пути к файлу.

**Возвращает:**
boolean — включать ли полный путь к файлу.
...
| Параметр | Тип | Описание |
| --- | --- | --- |
| value | boolean | Включать ли полный путь к файлу. |
