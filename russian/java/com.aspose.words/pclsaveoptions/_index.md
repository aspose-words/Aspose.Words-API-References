...
type: docs
weight: 448
url: /ru/java/com.aspose.words/pclsaveoptions/
---
...
```
 ...
 Может использоваться для указания дополнительных параметров при сохранении документа в[SaveFormat.PCL](../../com.aspose.words/saveformat\#PCL) формат.
...
| Метод | Описание |
| --- | --- |
| [addPrinterFont(String fontFullName, String fontPclName)](#addPrinterFont-java.lang.String-java.lang.String-) | Добавляет информацию о шрифте, загруженном производителем на принтер. |
...
| [getFallbackFontName()](#getFallbackFontName--) | Имя шрифта, который будет использоваться, если ожидаемый шрифт не найден в коллекциях принтеров и встроенных шрифтов. |
...
| [getRasterizeTransformedElements()](#getRasterizeTransformedElements--) | Получает значение, определяющее, следует ли растрировать сложные преобразованные элементы перед сохранением в документ PCL. |
...
| [setFallbackFontName(String value)](#setFallbackFontName-java.lang.String-) | Имя шрифта, который будет использоваться, если ожидаемый шрифт не найден в коллекциях принтеров и встроенных шрифтов. |
...
| [setRasterizeTransformedElements(boolean value)](#setRasterizeTransformedElements-boolean-) | Задает значение, определяющее, следует ли растрировать сложные трансформированные элементы перед сохранением в документ PCL. |
...
| --- | --- | --- |
| fontFullName | java.lang.String | Полное название шрифта (например, «Times New Roman Bold Italic»). |
| fontPclName | java.lang.String | Имя шрифта, используемого в документе Pcl. Есть 52 шрифта, которые должны быть встроены в любой принтер в соответствии со спецификацией Pcl. Однако производители могут добавлять в свои устройства некоторые другие шрифты. |
...
Имя шрифта, который будет использоваться, если ожидаемый шрифт не найден в коллекциях принтеров и встроенных шрифтов. Если резервный вариант не найден, генерируется предупреждение и используется шрифт «Arial».
...
Получает значение, определяющее, следует ли растрировать сложные преобразованные элементы перед сохранением в документ PCL. По умолчанию верно. PCL не поддерживает некоторые преобразования, используемые Aspose Words. Например, повернутые, перекошенные изображения и текстурные кисти. Для правильного рендеринга таких элементов используется процесс растеризации, т.е. сохранение в изображение и обрезка. Этот процесс может занять дополнительное время и память. Если для флага установлено значение false , некоторое содержимое в выводе может отличаться от исходного документа.

**Возвращает:**
boolean — значение, определяющее, следует ли растрировать сложные трансформированные элементы перед сохранением в документ PCL.
...
```
 ...
 Указывает формат, в котором будет сохранен документ, если используется этот объект параметров сохранения. Может быть только[SaveFormat.PCL](../../com.aspose.words/saveformat\#PCL).
...
Задает значение, определяющее, следует ли растрировать сложные трансформированные элементы перед сохранением в документ PCL. По умолчанию верно. PCL не поддерживает некоторые преобразования, используемые Aspose Words. Например, повернутые, перекошенные изображения и текстурные кисти. Для правильного рендеринга таких элементов используется процесс растеризации, т.е. сохранение в изображение и обрезка. Этот процесс может занять дополнительное время и память. Если для флага установлено значение false , некоторое содержимое в выводе может отличаться от исходного документа.

**Параметры:**

| Параметр | Тип | Описание |
| --- | --- | --- |
| value | boolean | Значение, определяющее, следует ли растрировать сложные трансформированные элементы перед сохранением в документ PCL. |
