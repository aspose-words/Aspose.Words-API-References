...
type: docs
weight: 496
url: /ru/java/com.aspose.words/rtfsaveoptions/
---
...
```
 ...
 Может использоваться для указания дополнительных параметров при сохранении документа в[SaveFormat.RTF](../../com.aspose.words/saveformat\#RTF) формат.
...
| [getExportCompactSize()](#getExportCompactSize--) | Позволяет уменьшить размер выходных RTF-документов, но если они содержат текст RTL (справа налево), он не будет отображаться корректно. |
...
| [getSaveImagesAsWmf()](#getSaveImagesAsWmf--) | При значении true все изображения будут сохранены в формате WMF. |
...
| [setExportCompactSize(boolean value)](#setExportCompactSize-boolean-) | Позволяет уменьшить размер выходных RTF-документов, но если они содержат текст RTL (справа налево), он не будет отображаться корректно. |
...
| [setSaveImagesAsWmf(boolean value)](#setSaveImagesAsWmf-boolean-) | При значении true все изображения будут сохранены в формате WMF. |
...
Позволяет уменьшить размер выходных RTF-документов, но если они содержат текст RTL (справа налево), он не будет отображаться корректно. Значение по умолчанию — ложь.

Если документ, который вы хотите преобразовать в RTF с помощью Aspose.Words, не содержит текста с написанием справа налево на таких языках, как арабский, вы можете установить для этого параметра значение true, чтобы уменьшить размер результирующего RTF.
...
Указывает, записываются ли ключевые слова для «старых читателей» в RTF или нет. Это может существенно повлиять на размер RTF-документа. Значение по умолчанию — true.

«Старые ридеры» — это приложения до Microsoft Word 97, а также WordPad. Когда этот параметр установлен, Aspose.Words записывает дополнительные ключевые слова RTF. Эти ключевые слова позволяют корректно отображать документ при открытии в «старом приложении для чтения», но могут значительно увеличить размер документа.

Если установить для этой опции значение false, то в «старых ридерах» будут отображаться только изображения в форматах WMF, EMF и BMP.
...
```
 ...
 Указывает формат, в котором будет сохранен документ, если используется этот объект параметров сохранения. Может быть только[SaveFormat.RTF](../../com.aspose.words/saveformat\#RTF).
...
При значении true все изображения будут сохранены в формате WMF. Этот параметр может помочь избежать появления предупреждающих сообщений WordPad.
