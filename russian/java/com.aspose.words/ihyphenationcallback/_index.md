...
description: "Реализуется классами, которые могут регистрировать словари переноса в Java."
type: docs
weight: 772
...
| Метод | Описание |
| --- | --- |
| [requestDictionary(String language)](#requestDictionary-java.lang.String) | Уведомляет приложение, что словарь переноса для указанного языка не найден и может потребоваться его регистрация. |
...
Уведомляет приложение, что словарь переноса для указанного языка не найден и может потребоваться его регистрация.

Реализация должна найти словарь и зарегистрировать его с помощью методов **M:Aspose.Words.Hyphenation.RegisterDictionary(System.String,System.IO.Stream)**.

Если словарь недоступен для указанного языка, реализация может отказаться от дальнейших вызовов для того же языка, используя [Hyphenation.registerDictionary(java.lang.String, java.lang.String)](../../com.aspose.words/hyphenation/\#registerDictionary-java.lang.String--java.lang.String) со значением  null .

 **Remarks:** 

Исключения, выброшенные этим методом, прервут выполнение процесса верстки страницы.
