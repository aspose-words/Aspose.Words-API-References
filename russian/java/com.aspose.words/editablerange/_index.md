...
description: "Представляет один редактируемый диапазон в Java."
type: docs
weight: 179
...
| [getEditorGroup()](#getEditorGroup) | Получает псевдоним (или группу редактирования), который будет использоваться для определения, может ли текущий пользователь редактировать этот редактируемый диапазон. |
| [getId()](#getId) | Получает идентификатор редактируемого диапазона. |
| [getSingleUser()](#getSingleUser) | Получает единственного пользователя для редактируемого диапазона. |
| [remove()](#remove) | Удаляет редактируемый диапазон из документа. |
| [setEditorGroup(int value)](#setEditorGroup-int) | Устанавливает псевдоним (или группу редактирования), который будет использоваться для определения, может ли текущий пользователь редактировать этот редактируемый диапазон. |
| [setSingleUser(String value)](#setSingleUser-java.lang.String) | Устанавливает единственного пользователя для редактируемого диапазона. |
...
Получает псевдоним (или группу редактирования), который будет использоваться для определения, может ли текущий пользователь редактировать этот редактируемый диапазон.

 **Remarks:** 

Единственный пользователь и группа редакторов не могут быть установлены одновременно для конкретного редактируемого диапазона; если один из них установлен, другой будет очищен.
...
**Returns:**
int — Псевдоним (или группа редактирования), который будет использоваться для определения, может ли текущий пользователь редактировать этот редактируемый диапазон. Возвращаемое значение является одной из констант [EditorType](../../com.aspose.words/editortype/).
### getId() {#getId}
```
public int getId()
```
...
Область должна быть размечена с использованием [getEditableRangeStart()](../../com.aspose.words/editablerange/\#getEditableRangeStart) и [getEditableRangeEnd()](../../com.aspose.words/editablerange/\#getEditableRangeEnd)

Идентификаторы редактируемых диапазонов должны быть уникальными в пределах документа, и Aspose.Words автоматически поддерживает их уникальность при загрузке, сохранении и объединении документов.
...
**Returns:**
int - Идентификатор редактируемого диапазона.
...
Получает единственного пользователя для редактируемого диапазона.

 **Remarks:** 

Этот редактор может быть сохранён в одной из следующих форм:

DOMAIN\\Username - для пользователей, доступ которых будет аутентифицирован с использованием доменных учётных данных текущего пользователя.

user@domain.com - для пользователей, доступ которых будет аутентифицирован с использованием адреса электронной почты пользователя в качестве учётных данных.

user - для пользователей, доступ которых будет аутентифицирован с использованием учётных данных машины текущего пользователя.
...
Устанавливает псевдоним (или группу редактирования), который будет использоваться для определения, может ли текущий пользователь редактировать этот редактируемый диапазон.
...
| --- | --- | --- |
| value | int | Псевдоним (или группа редактирования), который будет использоваться для определения, может ли текущий пользователь редактировать этот редактируемый диапазон. Значение должно быть одной из констант [EditorType](../../com.aspose.words/editortype/). |
...
Устанавливает единственного пользователя для редактируемого диапазона.
...
| Параметр | Тип | Описание |
| --- | --- | --- |
| значение | java.lang.String | Единственный пользователь для редактируемого диапазона. |
