...
description: Представляет вложенный документ, который является ссылкой на внешне хранящийся документ.
type: docs
weight: 540
...
```
 ...
 Представляет**SubDocument** \- который является ссылкой на внешне сохраненный документ.

 Чтобы узнать больше, посетите**Aspose.Words Document Object Model (DOM)** документальная статья.

 В этой версии Aspose.Words[SubDocument](../../com.aspose.words/subdocument) узлы не предоставляют общедоступных методов и свойств для создания или изменения вложенного документа. В этой версии вы не можете создавать экземпляры узлов SubDocument или изменять существующие, за исключением их удаления.

[SubDocument](../../com.aspose.words/subdocument) может быть только ребенком[Paragraph](../../com.aspose.words/paragraph).
## Методы
...
| [getNodeType()](#getNodeType--) |  Возвращает**NodeType.SubDocument** |
...
**Возвращает:**
 инт -**NodeType.SubDocument** Возвращаемое значение является одним из[NodeType](../../com.aspose.words/nodetype) константы.
