...
description: "Представляет поле документа Microsoft Word на Java."
type: docs
weight: 194
url: /ru/java/com.aspose.words/field/
---
...
Поле в документе Word — это сложная структура, состоящая из нескольких узлов, включающих начало поля, код поля, разделитель поля, результат поля и конец поля. Поля могут быть вложенными, содержать богатый контент и охватывать несколько абзацев или разделов в документе. Класс [Field](../../com.aspose.words/field/) является объектом "фасадом", предоставляющим свойства и методы, позволяющие работать с полем как с единственным объектом.

Свойства [getStart()](../../com.aspose.words/field/\#getStart), [getSeparator()](../../com.aspose.words/field/\#getSeparator) и [getEnd()](../../com.aspose.words/field/\#getEnd) указывают соответственно на узлы начала поля, разделителя и конца поля.

Содержимое между началом поля и разделителем является кодом поля. Содержимое между разделителем поля и концом поля является результатом поля. Код поля обычно состоит из одного или нескольких объектов [Run](../../com.aspose.words/run/) , которые задают инструкции. Ожидается, что приложение‑обработчик выполнит код поля для вычисления результата поля.

Процесс вычисления результатов полей называется обновлением поля. Aspose.Words может обновлять результаты полей большинства типов полей точно так же, как это делает Microsoft Word. Особенно, Aspose.Words может вычислять результаты даже самых сложных полей формул. Чтобы вычислить результат отдельного поля, используйте метод [update()](../../com.aspose.words/field/\#update). Чтобы обновить поля во всём документе, используйте [Document.updateFields()](../../com.aspose.words/document/\#updateFields).

Вы можете получить текстовую версию кода поля, используя метод [getFieldCode(boolean)](../../com.aspose.words/field/\#getFieldCode-boolean). Вы можете получить и установить текстовую версию результата поля, используя свойство [getResult()](../../com.aspose.words/field/\#getResult) / [setResult(java.lang.String)](../../com.aspose.words/field/\#setResult-java.lang.String). И код поля, и результат поля могут содержать сложное содержимое, такое как вложенные поля, абзацы, фигуры, таблицы, и в этом случае вы можете захотеть работать напрямую с узлами поля, если вам нужен больший контроль.

Вы не создаёте экземпляры класса [Field](../../com.aspose.words/field/) напрямую. Чтобы создать новое поле, используйте метод [DocumentBuilder.insertField(java.lang.String)](../../com.aspose.words/documentbuilder/\#insertField-java.lang.String).
