...
Представляет поле документа Microsoft Word.
 ...
 Чтобы узнать больше, посетите[ Working with Fields ][Working with Fields] документальная статья.

 Поле в документе Word представляет собой сложную структуру, состоящую из нескольких узлов, включая начало поля, код поля, разделитель полей, результат поля и конец поля. Поля могут быть вложенными, содержать форматированное содержимое и охватывать несколько абзацев или разделов документа.[Field](../../com.aspose.words/field) class — это «фасадный» объект, предоставляющий свойства и методы, позволяющие работать с полем как с единым объектом.

[getStart()](../../com.aspose.words/field\#getStart--), [getSeparator()](../../com.aspose.words/field\#getSeparator--) а также[getEnd()](../../com.aspose.words/field\#getEnd--) свойства указывают на начало поля, разделитель и конечный узел поля соответственно.

Содержимое между началом поля и разделителем является кодом поля. Содержимое между разделителем полей и концом поля является результатом поля. Код поля обычно состоит из одного или нескольких[Run](../../com.aspose.words/run) объекты, которые определяют инструкции. Ожидается, что приложение обработки выполнит код поля для вычисления результата поля.

 Процесс подсчета полевых результатов называется обновлением поля. Aspose.Words может обновлять результаты большинства типов полей точно так же, как это делает Microsoft Word. В частности, Aspose.Words может вычислять результаты даже самых сложных полей формул. Чтобы вычислить результат поля одного поля, используйте[update()](../../com.aspose.words/field\#update--) метод. Чтобы обновить поля во всем документе, используйте[Document.updateFields()](../../com.aspose.words/document\#updateFields--).

 Вы можете получить текстовую версию кода поля, используя[getFieldCode(boolean)](../../com.aspose.words/field\#getFieldCode-boolean-) метод. Вы можете получить и установить текстовую версию результата поля, используя[getResult()](../../com.aspose.words/field\#getResult--) / [setResult(java.lang.String)](../../com.aspose.words/field\#setResult-java.lang.String-)имущество. И код поля, и результат поля могут содержать сложное содержимое, такое как вложенные поля, абзацы, фигуры, таблицы, и в этом случае вы можете работать с узлами поля напрямую, если вам нужно больше контроля.

 Вы не создаете экземпляры[Field](../../com.aspose.words/field) класс напрямую. Чтобы создать новое поле, используйте[DocumentBuilder.insertField(java.lang.String)](../../com.aspose.words/documentbuilder\#insertField-java.lang.String-) метод.
