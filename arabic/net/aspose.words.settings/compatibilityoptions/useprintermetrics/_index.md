---
title: UsePrinterMetrics
second_title: Aspose.Words لمراجع .NET API
description: استخدام مقاييس الطابعة لعرض المستندات.
type: docs
weight: 640
url: /ar/net/aspose.words.settings/compatibilityoptions/useprintermetrics/
---
## CompatibilityOptions.UsePrinterMetrics property

استخدام مقاييس الطابعة لعرض المستندات.

```csharp
public bool UsePrinterMetrics { get; set; }
```

### ملاحظات

قد تختلف مقاييس الطابعة اعتمادًا على برامج التشغيل المستخدمة . على سبيل المثال ، يوفر Windows "Microsoft OpenXPS Class Driver 2" و "Microsoft Print to PDF" مقاييس مختلفة قليلاً ._ x000d_ لذلك ، قد يتغير تخطيط المستند النهائي إذا تم تمكين هذا الخيار ._ x000d_

### أنظر أيضا

* class [CompatibilityOptions](../../compatibilityoptions)
* مساحة الاسم [Aspose.Words.Settings](../../compatibilityoptions)
* المجسم [Aspose.Words](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Words.dll -->
