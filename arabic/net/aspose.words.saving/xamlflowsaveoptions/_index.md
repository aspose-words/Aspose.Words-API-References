...
description: Aspose.Words.Saving.XamlFlowSaveOptions فصل. يمكن استخدامها لتحديد خيارات إضافية عند حفظ مستند في XamlFlow أوXamlFlowPack التنسيق .
type: docs
weight: 5420
...
يمكن استخدامها لتحديد خيارات إضافية عند حفظ مستند في XamlFlow أوXamlFlowPack التنسيق .
...
| اسم | وصف |
| --- | --- |
| [XamlFlowSaveOptions](xamlflowsaveoptions/#constructor)() | يقوم بتهيئة مثيل جديد من هذه الفئة يمكن استخدامه لحفظ مستند بتنسيقXamlFlow التنسيق . |
| [XamlFlowSaveOptions](xamlflowsaveoptions/#constructor_1)(SaveFormat) | يقوم بتهيئة مثيل جديد من هذه الفئة يمكن استخدامه لحفظ مستند بتنسيقXamlFlow أوXamlFlowPack التنسيق . |
...
| [ImagesFolder](../../aspose.words.saving/xamlflowsaveoptions/imagesfolder/) { get; set; } | يحدد المجلد الفعلي حيث يتم حفظ الصور عند تصدير مستند إلى تنسيق XAML. الافتراضي هو عبارة عن سلسلة فارغة. |
| [ImagesFolderAlias](../../aspose.words.saving/xamlflowsaveoptions/imagesfolderalias/) { get; set; } | يحدد اسم المجلد المستخدم في إنشاء عناوين URI للصورة مكتوبة في مستند XAML . الافتراضي هو عبارة عن سلسلة فارغة. |
...
| override [SaveFormat](../../aspose.words.saving/xamlflowsaveoptions/saveformat/) { get; set; } | يحدد التنسيق الذي سيتم حفظ المستند به إذا تم استخدام كائن خيارات الحفظ . يمكن فقط أن يكونXamlFlow . |
