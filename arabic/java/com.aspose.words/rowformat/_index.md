...
description: "يمثل جميع تنسيقات صف الجدول في Java."
type: docs
weight: 590
url: /ar/java/com.aspose.words/rowformat/
---
...
| طريقة | الوصف |
| --- | --- |
| [clearFormatting()](#clearFormatting) | يعيد تعيين تنسيق الصف إلى الإعدادات الافتراضية. |
| [fetchInheritedBorderAttr(int key)](#fetchInheritedBorderAttr-int) |  |
| [getAllowBreakAcrossPages()](#getAllowBreakAcrossPages) | صحيح إذا كان مسموحًا للنص في صف الجدول أن ينقسم عبر فاصل الصفحة. |
| [getBorders()](#getBorders) | يحصل على مجموعة حدود الخلايا الافتراضية للصف. |
| [getDirectBorderAttr(int key)](#getDirectBorderAttr-int) |  |
| [getHeadingFormat()](#getHeadingFormat) | صحيح إذا تم تكرار الصف كعنوان للجدول في كل صفحة عندما يمتد الجدول على أكثر من صفحة. |
| [getHeight()](#getHeight) | يحصل على ارتفاع صف الجدول بالنقاط. |
| [getHeightRule()](#getHeightRule) | يحصل على القاعدة لتحديد ارتفاع صف الجدول. |
| [setAllowBreakAcrossPages(boolean value)](#setAllowBreakAcrossPages-boolean) | صحيح إذا كان مسموحًا للنص في صف الجدول أن ينقسم عبر فاصل الصفحة. |
| [setBorderAttr(int key, Object value)](#setBorderAttr-int-java.lang.Object) |  |
| [setHeadingFormat(boolean value)](#setHeadingFormat-boolean) | صحيح إذا تم تكرار الصف كعنوان للجدول في كل صفحة عندما يمتد الجدول على أكثر من صفحة. |
| [setHeight(double value)](#setHeight-double) | يضبط ارتفاع صف الجدول بالنقاط. |
| [setHeightRule(int value)](#setHeightRule-int) | يضبط القاعدة لتحديد ارتفاع صف الجدول. |
...
يعيد تعيين تنسيق الصف إلى الإعدادات الافتراضية.
...
صحيح إذا كان مسموحًا للنص في صف الجدول أن ينقسم عبر فاصل الصفحة.

 **Examples:** 

يوضح كيفية تعطيل انقسام الصفوف عبر الصفحات لكل صف في جدول.
...
يحصل على مجموعة حدود الخلايا الافتراضية للصف.
...
صحيح إذا تم تكرار الصف كعنوان للجدول في كل صفحة عندما يمتد الجدول على أكثر من صفحة.

 **Examples:** 

يوضح كيفية بناء جدول يحتوي على صفوف تتكرر في كل صفحة.
...
يحصل على ارتفاع صف الجدول بالنقاط.
...
**Returns:**
double - ارتفاع صف الجدول بالنقاط.
...
يحصل على القاعدة لتحديد ارتفاع صف الجدول.
...
**Returns:**
int - القاعدة لتحديد ارتفاع صف الجدول. القيمة المرجعة هي واحدة من ثوابت [HeightRule](../../com.aspose.words/heightrule/).
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| قيمة | double | ارتفاع صف الجدول بالنقاط. |
...
يضبط القاعدة لتحديد ارتفاع صف الجدول.
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| value | int | القاعدة لتحديد ارتفاع صف الجدول. يجب أن تكون القيمة واحدة من ثوابت [HeightRule](../../com.aspose.words/heightrule/). |
