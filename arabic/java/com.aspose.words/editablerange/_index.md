...
description: "يمثل نطاقًا قابلاً للتحرير واحدًا في Java."
type: docs
weight: 179
url: /ar/java/com.aspose.words/editablerange/
---
...
يمثل نطاقًا قابلًا للتحرير واحدًا.
...
| طريقة | الوصف |
| --- | --- |
| [getEditableRangeEnd()](#getEditableRangeEnd) | يحصل على العقدة التي تمثل نهاية النطاق القابل للتحرير. |
| [getEditableRangeStart()](#getEditableRangeStart) | يحصل على العقدة التي تمثل بداية النطاق القابل للتحرير. |
| [getEditorGroup()](#getEditorGroup) | يحصل على اسم مستعار (أو مجموعة تحرير) يُستخدم لتحديد ما إذا كان المستخدم الحالي مسموحًا له بتحرير هذا النطاق القابل للتحرير. |
| [getId()](#getId) | يحصل على معرف النطاق القابل للتحرير. |
| [getSingleUser()](#getSingleUser) | يحصل على المستخدم الفردي للنطاق القابل للتحرير. |
| [remove()](#remove) | يزيل النطاق القابل للتحرير من المستند. |
| [setEditorGroup(int value)](#setEditorGroup-int) | يضبط اسمًا مستعارًا (أو مجموعة تحرير) يُستخدم لتحديد ما إذا كان المستخدم الحالي مسموحًا له بتحرير هذا النطاق القابل للتحرير. |
| [setSingleUser(String value)](#setSingleUser-java.lang.String) | يضبط المستخدم الفردي للنطاق القابل للتحرير. |
...
يحصل على اسم مستعار (أو مجموعة تحرير) يُستخدم لتحديد ما إذا كان المستخدم الحالي مسموحًا له بتحرير هذا النطاق القابل للتحرير.

 **Remarks:** 

لا يمكن تعيين المستخدم الفردي ومجموعة المحررين في نفس الوقت للنطاق القابل للتحرير المحدد؛ إذا تم تعيين أحدهما، سيُمسح الآخر.
...
**Returns:**
int - اسم مستعار (أو مجموعة تحرير) يُستخدم لتحديد ما إذا كان المستخدم الحالي مسموحًا له بتحرير هذا النطاق القابل للتحرير. القيمة المرجعة هي واحدة من ثوابت [EditorType](../../com.aspose.words/editortype/).
### getId() {#getId}
```
public int getId()
```
...
يجب تحديد المنطقة باستخدام [getEditableRangeStart()](../../com.aspose.words/editablerange/\\#getEditableRangeStart) و [getEditableRangeEnd()](../../com.aspose.words/editablerange/\\#getEditableRangeEnd)

من المفترض أن تكون معرفات النطاق القابل للتحرير فريدة عبر المستند، وتقوم Aspose.Words تلقائيًا بالحفاظ على معرفات النطاق القابل للتحرير عند تحميل المستندات وحفظها ودمجها.
...
**Returns:**
int - معرف النطاق القابل للتحرير.
...
يحصل على المستخدم الفردي للنطاق القابل للتحرير.
...
**Returns:**
java.lang.String - المستخدم الفردي للنطاق القابل للتحرير.
### remove() {#remove}
```
public void remove()
```
...
يضبط اسمًا مستعارًا (أو مجموعة تحرير) يُستخدم لتحديد ما إذا كان المستخدم الحالي مسموحًا له بتحرير هذا النطاق القابل للتحرير.
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| value | int | اسم مستعار (أو مجموعة تحرير) سيُستخدم لتحديد ما إذا كان يُسمح للمستخدم الحالي بتحرير هذا النطاق القابل للتحرير. يجب أن تكون القيمة واحدة من ثوابت [EditorType](../../com.aspose.words/editortype/). |
...
يضبط المستخدم الفردي للنطاق القابل للتحرير.
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| قيمة | java.lang.String | المستخدم الفردي للنطاق القابل للتحرير. |
