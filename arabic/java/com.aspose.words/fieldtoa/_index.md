...
description: "ينفّذ حقل TOA في Java."
type: docs
weight: 297
url: /ar/java/com.aspose.words/fieldtoa/
---
...
يبني جدولًا للسلطات (أي قائمة بالمراجع في مستند قانوني، مثل المراجع إلى القضايا، القوانين، والأنظمة، مع أرقام الصفحات التي تظهر فيها المراجع) باستخدام الإدخالات المحددة بواسطة حقول TA.

 **Examples:** 

يظهر كيفية بناء وتخصيص جدول للسلطات باستخدام حقول TOA و TA.
...
| [getEntryCategory()](#getEntryCategory) | يحصل على الفئة الكاملة للمدخلات المتضمنة في الجدول. |
| [getEntrySeparator()](#getEntrySeparator) | يحصل على تسلسل الأحرف المستخدم لفصل إدخال جدول المراجع ورقم صفحته. |
...
| [getRemoveEntryFormatting()](#getRemoveEntryFormatting) | يحصل على ما إذا كان يجب إزالة تنسيق نص الإدخال في المستند من الإدخال في جدول المراجع. |
...
| [getUseHeading()](#getUseHeading) | يحصل على ما إذا كان يجب تضمين عنوان الفئة للمدخلات في جدول المراجع. |
| [getUsePassim()](#getUsePassim) | يحصل على ما إذا كان يجب استبدال خمس إشارات صفحة مختلفة أو أكثر لنفس المرجع بـ \"passim\", والذي يُستخدم للدلالة على أن كلمة أو مقطع يظهر بشكل متكرر في العمل المستشهد به. |
...
| [setEntryCategory(String value)](#setEntryCategory-java.lang.String) | يضبط الفئة الكاملة للمدخلات المتضمنة في الجدول. |
| [setEntrySeparator(String value)](#setEntrySeparator-java.lang.String) | يضبط تسلسل الأحرف المستخدم لفصل إدخال جدول المراجع ورقم صفحته. |
...
| [setRemoveEntryFormatting(boolean value)](#setRemoveEntryFormatting-boolean) | يضبط ما إذا كان يجب إزالة تنسيق نص الإدخال في المستند من الإدخال في جدول المراجع. |
...
| [setUseHeading(boolean value)](#setUseHeading-boolean) | يضبط ما إذا كان يجب تضمين عنوان الفئة للمدخلات في جدول المراجع. |
| [setUsePassim(boolean value)](#setUsePassim-boolean) | يضبط ما إذا كان يجب استبدال خمس إشارات صفحة مختلفة أو أكثر لنفس المرجع بـ \"passim\", والذي يُستخدم للدلالة على أن كلمة أو مقطع يظهر بشكل متكرر في العمل المستشهد به. |
...
يحصل على الفئة الكاملة للمدخلات المتضمنة في الجدول.
...
**Returns:**
java.lang.String - الفئة الكاملة للمدخلات المتضمنة في الجدول.
...
يحصل على تسلسل الأحرف المستخدم لفصل إدخال جدول المراجع ورقم صفحته.
...
**Returns:**
java.lang.String - تسلسل الأحرف المستخدم لفصل إدخال جدول المراجع ورقم صفحته.
...
يحصل على ما إذا كان يجب إزالة تنسيق نص الإدخال في المستند من الإدخال في جدول المراجع.
...
**Returns:**
boolean - ما إذا كان يجب إزالة تنسيق نص الإدخال في المستند من الإدخال في جدول المراجع.
### getResult() {#getResult}
```
...
يحصل على ما إذا كان يجب تضمين عنوان الفئة للمدخلات في جدول المراجع.
...
**Returns:**
boolean - ما إذا كان يجب تضمين عنوان الفئة للإدخالات في جدول المراجع.
...
يحصل على ما إذا كان يجب استبدال خمس إشارات صفحة مختلفة أو أكثر لنفس المرجع بـ \"passim\", والذي يُستخدم للدلالة على أن كلمة أو مقطع يظهر بشكل متكرر في العمل المستشهد به.
...
**Returns:**
boolean - ما إذا كان يجب استبدال خمسة مراجع صفحات مختلفة أو أكثر لنفس المرجع بـ "passim"، والذي يُستخدم للدلالة على أن كلمة أو مقطع يتكرر كثيرًا في العمل المستشهد به.
### isDirty() {#isDirty}
```
...
يضبط الفئة الكاملة للمدخلات المتضمنة في الجدول.
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| قيمة | java.lang.String | الفئة المتكاملة للإدخالات المشمولة في الجدول. |
...
يضبط تسلسل الأحرف المستخدم لفصل إدخال جدول المراجع ورقم صفحته.
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| قيمة | java.lang.String | تسلسل الأحرف المستخدم لفصل إدخال جدول المراجع ورقم صفحته. |
...
يضبط ما إذا كان يجب إزالة تنسيق نص الإدخال في المستند من الإدخال في جدول المراجع.
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| قيمة | boolean | ما إذا كان يجب إزالة تنسيق نص الإدخال في المستند من الإدخال في جدول المراجع. |
...
يضبط ما إذا كان يجب تضمين عنوان الفئة للمدخلات في جدول المراجع.
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| قيمة | boolean | ما إذا كان يجب تضمين عنوان الفئة للإدخالات في جدول المراجع. |
...
يضبط ما إذا كان يجب استبدال خمس إشارات صفحة مختلفة أو أكثر لنفس المرجع بـ \"passim\", والذي يُستخدم للدلالة على أن كلمة أو مقطع يظهر بشكل متكرر في العمل المستشهد به.
...
| معامل | نوع | الوصف |
| --- | --- | --- |
| قيمة | boolean | ما إذا كان يجب استبدال خمسة مراجع صفحات مختلفة أو أكثر لنفس المرجع بـ "passim"، والذي يُستخدم للدلالة على أن كلمة أو مقطع يتكرر كثيرًا في العمل المستشهد به. |
