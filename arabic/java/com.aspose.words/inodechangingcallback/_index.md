...
description: "نفّذ هذه الواجهة إذا كنت تريد تلقي الإشعارات عندما يتم إدراج العقد أو إزالتها في المستند في جافا."
type: docs
weight: 778
url: /ar/java/com.aspose.words/inodechangingcallback/
---
```
public interface INodeChangingCallback
```

قم بتنفيذ هذه الواجهة إذا كنت تريد تلقي الإشعارات عندما يتم إدراج أو إزالة العقد في المستند.
...
| طريقة | الوصف |
| --- | --- |
| [nodeInserted(NodeChangingArgs args)](#nodeInserted-com.aspose.words.NodeChangingArgs) | يُستدعى عندما يتم إدراج عقدة تابعة لهذا المستند في عقدة أخرى. |
| [nodeInserting(NodeChangingArgs args)](#nodeInserting-com.aspose.words.NodeChangingArgs) | يُستدعى قبل مباشرة أن يتم إدراج عقدة تابعة لهذا المستند في عقدة أخرى. |
| [nodeRemoved(NodeChangingArgs args)](#nodeRemoved-com.aspose.words.NodeChangingArgs) | يُستدعى عندما يتم إزالة عقدة تابعة لهذا المستند من أصلها. |
| [nodeRemoving(NodeChangingArgs args)](#nodeRemoving-com.aspose.words.NodeChangingArgs) | يتم استدعاؤه مباشرةً قبل أن تتم إزالة عقدة تابعة لهذا المستند من المستند. |
...
يُستدعى عندما يتم إدراج عقدة تابعة لهذا المستند في عقدة أخرى.
...
يُستدعى قبل مباشرة أن يتم إدراج عقدة تابعة لهذا المستند في عقدة أخرى.
...
يُستدعى عندما يتم إزالة عقدة تابعة لهذا المستند من أصلها.
...
يتم استدعاؤه مباشرةً قبل أن تتم إزالة عقدة تابعة لهذا المستند من المستند.
