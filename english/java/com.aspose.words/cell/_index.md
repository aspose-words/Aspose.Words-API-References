...
| [getCellFormat()](#getCellFormat--) | 提供对单元格格式属性的访问。 |
...
| [getFirstParagraph()](#getFirstParagraph--) | 获取直接子级中的第一段。 |
| [getLastChild()](#getLastChild--) | 获取节点的最后一个子节点。 |
| [getLastParagraph()](#getLastParagraph--) | 获取直接子级中的最后一段。 |
| [getNextMatchingNode(Node curNode)](#getNextMatchingNode-com.aspose.words.Node-) |  |
| [getNextSibling()](#getNextSibling--) | 获取紧跟此节点的节点。 |
| [getNode类型()](#getNode类型--) | 退货**Node类型.Cell**. |
| [getParagraphs()](#getParagraphs--) | 获取作为单元格的直接子级的段落的集合。 |
| [getParentNode()](#getParentNode--) | 获取此节点的直接父节点。 |
| [getParentRow()](#getParentRow--) | 返回单元格的父行。 |
| [getPreviousSibling()](#getPreviousSibling--) | 获取紧接在此节点之前的节点。 |
| [getRange()](#getRange--) | 返回一个**Range**表示包含在此节点中的文档部分的对象。 |
| [getTables()](#getTables--) | 获取作为单元格的直接子级的表的集合。 |
...
| [isFirstCell()](#isFirstCell--) | 如果这是一行中的第一个单元格，则为真；否则为假。 |
| [isLastCell()](#isLastCell--) | 如果这是一行中的最后一个单元格，则为真；否则为假。 |
...
什么时候**Cell**已创建，它属于指定的文档，但还不是文档的一部分，并且**ParentNode**一片空白。

追加**Cell**在要插入单元格的行上使用 InsertAfter 或 InsertBefore 到文档中。
...
**退货:**
boolean - 如果所有节点都被访问，则为真；如果 DocumentVisitor 在访问所有节点之前停止操作，则返回 false。调用 DocumentVisitor.VisitCellStart，然后为该部分的所有子节点调用 Accept，最后调用 DocumentVisitor.VisitCellEnd。
...
**退货:**
[CellFormat](../../com.aspose.words/cellformat) - 相应的[CellFormat](../../com.aspose.words/cellformat)价值。
### getChild(int node类型, int index, boolean isDeep) {#getChild-int-int-boolean-}
```
...
**退货:**
[Paragraph](../../com.aspose.words/paragraph) - 直系子女中的第一段。
...
**退货:**
[Paragraph](../../com.aspose.words/paragraph) - 直系子女中的最后一段。
### getNextMatchingNode(Node curNode) {#getNextMatchingNode-com.aspose.words.Node-}
```
...
**退货:**
诠释 -**Node类型.Cell** .返回值是以下之一[Node类型](../../com.aspose.words/nodetype)常数。
...
**退货:**
[ParagraphCollection](../../com.aspose.words/paragraphcollection) - 作为单元格的直接子级的段落的集合。
...
返回单元格的父行。等效于 (Row)FirstNonMarkupParentNode 。

**退货:**
[Row](../../com.aspose.words/row) - 单元格的父行。
...
**退货:**
[TableCollection](../../com.aspose.words/tablecollection) - 作为单元格的直接子级的表的集合。
### getText() {#getText--}
```
