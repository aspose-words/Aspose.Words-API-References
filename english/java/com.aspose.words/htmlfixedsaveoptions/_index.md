...
指定添加到 style.css 文件中所有类名的前缀。默认值为 "aw" 。
...
指定字体是否应嵌入到 Base64 格式的 Html 文档中。注意设置此标志可以显着增加输出 Html 文件的大小。
...
**退货:**
boolean - 对应的布尔值。
### getExportForm字段() {#getExportForm字段--}
```
public boolean getExportForm字段()
```
...
**退货:**
int - 对应的 int 值。返回值是以下之一[HtmlFixedPageHorizontalAlignment](../../com.aspose.words/htmlfixedpagehorizontalalignment)常数。
...
仅在以下情况下有效[getExportEmbeddedImages()](../../com.aspose.words/htmlfixedsaveoptions\#getExportEmbeddedImages--) / [setExportEmbeddedImages(boolean)](../../com.aspose.words/htmlfixedsaveoptions\#setExportEmbeddedImages-boolean-)属性为假。
...
### setExportForm字段(boolean value) {#setExportForm字段-boolean-}
```
public void setExportForm字段(boolean value)
```
...
| 范围 | 类型 | 描述 |
| --- | --- | --- |
| value | int | 对应的 int 值。该值必须是以下之一[HtmlFixedPageHorizontalAlignment](../../com.aspose.words/htmlfixedpagehorizontalalignment)常数。 |
