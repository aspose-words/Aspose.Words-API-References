﻿---
title: BookmarkCollection class
second_title: Aspose.Words for Python via .NET API Reference
description: "A collection of [Bookmark](../bookmark/) objects that represent the bookmarks in the specified range"
type: docs
weight: 40
url: /python-net/aspose.words/bookmarkcollection/
---

## BookmarkCollection class

A collection of [Bookmark](../bookmark/) objects that represent the bookmarks in the specified range.
To learn more, visit the [Working with Bookmarks](https://docs.aspose.com/words/python-net/working-with-bookmarks/) documentation article.




### Indexers

| Name | Description |
| --- | --- |
| [``__getitem__(index)``](./__getitem__/#int) | Returns a bookmark at the specified index. |

### Properties

| Name | Description |
| --- | --- |
| [count](./count/) | Returns the number of bookmarks in the collection. |

### Methods

| Name | Description |
| --- | --- |
|[ clear()](./clear/#default) | Removes all bookmarks from this collection and from the document. |
|[ get_by_name(bookmark_name)](./get_by_name/#str) | Returns a bookmark by name. |
|[ remove(bookmark)](./remove/#bookmark) | Removes the specified bookmark from the document. |
|[ remove(bookmark_name)](./remove/#str) | Removes a bookmark with the specified name. |
|[ remove_at(index)](./remove_at/#int) | Removes a bookmark at the specified index. |

### See Also

* module [aspose.words](../)

