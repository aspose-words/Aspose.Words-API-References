---
title: read method
second_title: Aspose.Words for Python via .NET API Reference
description: "Reads the data from the specified stream into the [Document](../../document/) instance."
type: docs
weight: 10
url: /python-net/aspose.words/idocumentreaderplugin/read/
---

## read(src, load_options, document) {#bytesio_loadoptions_document}

Reads the data from the specified stream into the [Document](../../document/) instance.



```python
def read(self, src: BytesIO, load_options: aspose.words.loading.LoadOptions, document: aspose.words.Document):
    ...
```

| Parameter | Type | Description |
| --- | --- | --- |
| src | BytesIO |  |
| load_options | [LoadOptions](../../../aspose.words.loading/loadoptions/) |  |
| document | [Document](../../document/) |  |

### See Also

* module [aspose.words](../../)
* class [IDocumentReaderPlugin](../)

