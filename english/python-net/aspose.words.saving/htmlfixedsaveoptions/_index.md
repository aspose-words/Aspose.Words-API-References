...
description: "Can be used to specify additional options when saving a document into the [SaveFormat.HTML_FIXED](../../aspose.words/saveformat/#HTML_FIXED) format."
type: docs
weight: 230
...
| [page_horizontal_alignment](./page_horizontal_alignment/) | Specifies the horizontal alignment of pages in an HTML document. Default value is ``HtmlFixedHorizontalPageAlignment.Center``. |
...
| [resources_folder](./resources_folder/) | Specifies the physical folder where resources (images, fonts, css) are saved when exporting a document to Html format. Default is ``null``. |
| [resources_folder_alias](./resources_folder_alias/) | Specifies the name of the folder used to construct image URIs written into an Html document. Default is ``null``. |
