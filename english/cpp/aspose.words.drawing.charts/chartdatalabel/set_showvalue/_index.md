---
title: Aspose::Words::Drawing::Charts::ChartDataLabel::set_ShowValue method
linktitle: set_ShowValue
second_title: Aspose.Words for C++ API Reference
description: 'Aspose::Words::Drawing::Charts::ChartDataLabel::set_ShowValue method. Allows to specify if values are to be displayed in the data labels. Default value is false in C++.'
type: docs
weight: 28000
url: /cpp/aspose.words.drawing.charts/chartdatalabel/set_showvalue/
---
## ChartDataLabel::set_ShowValue method


Allows to specify if values are to be displayed in the data labels. Default value is **false**.

```cpp
void Aspose::Words::Drawing::Charts::ChartDataLabel::set_ShowValue(bool value)
```

## See Also

* Class [ChartDataLabel](../)
* Namespace [Aspose::Words::Drawing::Charts](../../)
* Library [Aspose.Words for C++](../../../)
