---
title: TwoColorGradient
second_title: Aspose.Words for .NET API Reference
description: 
type: docs
weight: 50
url: /net/aspose.words.drawing/fill/twocolorgradient/
---
## Fill.TwoColorGradient method (1 of 2)

Sets the specified fill to a two-color gradient.

```csharp
public void TwoColorGradient(GradientStyle style, GradientVariant variant)
```

| parameter | description |
| --- | --- |
| style | The gradient style [`GradientStyle`](../../gradientstyle). |
| variant | The gradient variant [`GradientVariant`](../../gradientvariant) |

### See Also

* enum [GradientStyle](../../gradientstyle)
* enum [GradientVariant](../../gradientvariant)
* class [Fill](../../fill)
* namespace [Aspose.Words.Drawing](../../fill)
* assembly [Aspose.Words](../../../)

---

## Fill.TwoColorGradient method (2 of 2)

Sets the specified fill to a two-color gradient.

```csharp
public void TwoColorGradient(Color color1, Color color2, GradientStyle style, 
    GradientVariant variant)
```

| parameter | description |
| --- | --- |
| color1 | The first gradient color, which will be set to [`ForeColor`](../forecolor) after the fill is applied. |
| color2 | The second gradient color, which will be set to [`BackColor`](../backcolor) after the fill is applied. |
| style | The gradient style [`GradientStyle`](../../gradientstyle). |
| variant | The gradient variant [`GradientVariant`](../../gradientvariant) |

### See Also

* enum [GradientStyle](../../gradientstyle)
* enum [GradientVariant](../../gradientvariant)
* class [Fill](../../fill)
* namespace [Aspose.Words.Drawing](../../fill)
* assembly [Aspose.Words](../../../)

<!-- DO NOT EDIT: generated by xmldocmd for Aspose.Words.dll -->
