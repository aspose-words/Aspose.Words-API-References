---
title: "Aspose::Words::Fields::FieldMergeBarcode klass"
linktitle: "FieldMergeBarcode"
second_title: "Aspose.Words för C++ API‑referens"
description: "Aspose::Words::Fields::FieldMergeBarcode klass. Implementerar MERGEBARCODE-fältet. För att lära dig mer, besök dokumentationsartikeln i C++."
type: docs
weight: 66000
url: /sv/cpp/aspose.words.fields/fieldmergebarcode/
---
## FieldMergeBarcode class


Implementerar MERGEBARCODE-fältet. För att lära dig mer, besök dokumentationsartikeln [Working with Fields](https://docs.aspose.com/words/cpp/working-with-fields/).

```cpp
class FieldMergeBarcode : public Aspose::Words::Fields::Field,
                          public Aspose::Words::Fields::IFieldCodeTokenInfoProvider,
                          public Aspose::Words::Fields::IMergeFieldSurrogate
```

## Metoder

| Metod | Beskrivning |
| --- | --- |
| [get_AddStartStopChar](./get_addstartstopchar/)() | Hämtar om Start/Stop-tecken ska läggas till för streckkodstyperna NW7 och CODE39. |
| [get_BackgroundColor](./get_backgroundcolor/)() | Hämtar bakgrundsfärgen för streckkodssymbolen. Giltiga värden ligger i intervallet [0, 0xFFFFFF]. |
| [get_BarcodeType](./get_barcodetype/)() | Hämtar streckkodstypen (QR, osv.) |
| [get_BarcodeValue](./get_barcodevalue/)() | Hämtar streckkodsvärdet. |
| [get_CaseCodeStyle](./get_casecodestyle/)() | Gets the style of a Case Code for barcode type ITF14. The valid values are [STD | EXT | ADD]. |
| [get_DisplayResult](../field/get_displayresult/)() | Hämtar texten som representerar det visade fältresultatet. |
| [get_DisplayText](./get_displaytext/)() | Hämtar om streckkodsdata (text) ska visas tillsammans med bilden. |
| [get_End](./get_end/)() override | Hämtar noden som representerar fältets slut. |
| [get_End](../field/get_end/)() const | Hämtar noden som representerar fältets slut. |
| [get_ErrorCorrectionLevel](./get_errorcorrectionlevel/)() | Hämtar felkorrigeringsnivån för QR-kod. Giltiga värden är [0, 3]. |
| [get_FieldEnd](../field/get_fieldend/)() const | Hämtar noden som representerar fältets slut. |
| [get_FieldStart](../field/get_fieldstart/)() const | Hämtar noden som representerar fältets början. |
| [get_FixCheckDigit](./get_fixcheckdigit/)() | Hämtar om kontrollsiffran ska korrigeras om den är ogiltig. |
| [get_ForegroundColor](./get_foregroundcolor/)() | Hämtar förgrundsfärgen för streckkodssymbolen. Giltiga värden ligger i intervallet [0, 0xFFFFFF]. |
| [get_Format](../field/get_format/)() | Hämtar ett [FieldFormat](../fieldformat/) objekt som ger typad åtkomst till fältets formatering. |
| [get_IsDirty](../field/get_isdirty/)() | Hämtar eller anger om det aktuella resultatet av fältet inte längre är korrekt (föråldrat) på grund av andra ändringar som gjorts i dokumentet. |
| [get_IsLocked](../field/get_islocked/)() | Hämtar eller anger om fältet är låst (bör inte beräkna om sitt resultat). |
| [get_LocaleId](../field/get_localeid/)() | Hämtar eller anger LCID för fältet. |
| [get_PosCodeStyle](./get_poscodestyle/)() | Gets the style of a Point of Sale barcode (barcode types UPCA | UPCE | EAN13 | EAN8). The valid values (case insensitive) are [STD | SUP2 | SUP5 | CASE]. |
| [get_Result](../field/get_result/)() | Hämtar eller anger text som ligger mellan fältavgränsaren och fältets slut. |
| [get_ScalingFactor](./get_scalingfactor/)() | Hämtar en skalningsfaktor för symbolen. Värdet är i hela procentenheter och giltiga värden är [10, 1000]. |
| [get_Separator](./get_separator/)() override | Hämtar noden som representerar fältavgränsaren. Kan vara **null**. |
| [get_Start](./get_start/)() override | Hämtar noden som representerar fältets början. |
| [get_Start](../field/get_start/)() const | Hämtar noden som representerar fältets början. |
| [get_SymbolHeight](./get_symbolheight/)() | Hämtar symbolens höjd. Enheterna är i TWIPS (1/1440 tum). |
| [get_SymbolRotation](./get_symbolrotation/)() | Hämtar rotationen för streckkodssymbolen. Giltiga värden är [0, 3]. |
| virtual [get_Type](../field/get_type/)() const | Hämtar Microsoft Word-fälttypen. |
| [GetFieldCode](../field/getfieldcode/)() | Returnerar text mellan fältets början och fältavgränsaren (eller fältets slut om det inte finns någon avgränsare). Både fältkod och fältresultat för underfält inkluderas. |
| [GetFieldCode](../field/getfieldcode/)(bool) | Returnerar text mellan fältets början och fältavgränsaren (eller fältets slut om det inte finns någon avgränsare). |
| [GetType](./gettype/)() const override |  |
| [Is](./is/)(const System::TypeInfo\&) const override |  |
| virtual [Remove](../field/remove/)() | Tar bort fältet från dokumentet. Returnerar en nod precis efter fältet. Om fältets slut är det sista barnet till dess föräldranod, returneras dess föräldrapparagraf. Om fältet redan har tagits bort, returneras **null**. |
| [set_AddStartStopChar](./set_addstartstopchar/)(bool) | Ställer in om Start/Stop-tecken ska läggas till för streckkodstyperna NW7 och CODE39. |
| [set_BackgroundColor](./set_backgroundcolor/)(const System::String\&) | Ställer in bakgrundsfärgen för streckkodssymbolen. Giltiga värden ligger i intervallet [0, 0xFFFFFF]. |
| [set_BarcodeType](./set_barcodetype/)(const System::String\&) | Ställer in streckkodstypen (QR, osv.) |
| [set_BarcodeValue](./set_barcodevalue/)(const System::String\&) | Ställer in streckkodsvärdet. |
| [set_CaseCodeStyle](./set_casecodestyle/)(const System::String\&) | Sets the style of a Case Code for barcode type ITF14. The valid values are [STD | EXT | ADD]. |
| [set_DisplayText](./set_displaytext/)(bool) | Ställer in om streckkoddata (text) ska visas tillsammans med bilden. |
| [set_ErrorCorrectionLevel](./set_errorcorrectionlevel/)(const System::String\&) | Ställer in en felkorrigeringsnivå för QR‑kod. Giltiga värden är [0, 3]. |
| [set_FixCheckDigit](./set_fixcheckdigit/)(bool) | Ställer in om kontrollsiffran ska korrigeras om den är ogiltig. |
| [set_ForegroundColor](./set_foregroundcolor/)(const System::String\&) | Ställer in förgrundsfärgen för streckkodssymbolen. Giltiga värden ligger i intervallet [0, 0xFFFFFF]. |
| [set_IsDirty](../field/set_isdirty/)(bool) | Sättare för [Aspose::Words::Fields::Field::get_IsDirty](../field/get_isdirty/). |
| [set_IsLocked](../field/set_islocked/)(bool) | Sättare för [Aspose::Words::Fields::Field::get_IsLocked](../field/get_islocked/). |
| [set_LocaleId](../field/set_localeid/)(int32_t) | Sättare för [Aspose::Words::Fields::Field::get_LocaleId](../field/get_localeid/). |
| [set_PosCodeStyle](./set_poscodestyle/)(const System::String\&) | Sets the style of a Point of Sale barcode (barcode types UPCA | UPCE | EAN13 | EAN8). The valid values (case insensitive) are [STD | SUP2 | SUP5 | CASE]. |
| [set_Result](../field/set_result/)(const System::String\&) | Sättare för [Aspose::Words::Fields::Field::get_Result](../field/get_result/). |
| [set_ScalingFactor](./set_scalingfactor/)(const System::String\&) | Ställer in en skalningsfaktor för symbolen. Värdet är i hela procentenheter och giltiga värden är [10, 1000]. |
| [set_SymbolHeight](./set_symbolheight/)(const System::String\&) | Ställer in symbolens höjd. Enheterna är i TWIPS (1/1440 tum). |
| [set_SymbolRotation](./set_symbolrotation/)(const System::String\&) | Ställer in rotationen för streckkodssymbolen. Giltiga värden är [0, 3]. |
| static [Type](./type/)() |  |
| [Unlink](../field/unlink/)() | Utför avlänkning av fältet. |
| [Update](../field/update/)() | Utför fältuppdateringen. Kastar ett undantag om fältet redan uppdateras. |
| [Update](../field/update/)(bool) | Utför en fältuppdatering. Kastar ett undantag om fältet redan uppdateras. |
## Se även

* Class [Field](../field/)
* Namespace [Aspose::Words::Fields](../)
* Library [Aspose.Words for C++](../../)
