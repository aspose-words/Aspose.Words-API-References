---
title: Enum PresetTexture
second_title: Aspose.Words för .NET API Referens
description: Aspose.Words.Drawing.PresetTexture uppräkning. Anger textur som ska användas för att fylla en form.
type: docs
weight: 1050
url: /sv/net/aspose.words.drawing/presettexture/
---
## PresetTexture enumeration

Anger textur som ska användas för att fylla en form.

```csharp
public enum PresetTexture
```

### Värderingar

| namn | Värde | Beskrivning |
| --- | --- | --- |
| None | `-1` | Ingen textur. |
| BlueTissuePaper | `1` | Blå silkespappersstruktur. |
| Bouquet | `2` | Bukettstruktur. |
| BrownMarble | `3` | Brun marmorstruktur. |
| Canvas | `4` | Canvas textur. |
| Cork | `5` | Korkstruktur. |
| Denim | `6` | Denimstruktur. |
| FishFossil | `7` | Fisk fossil textur. |
| Granite | `8` | Granitstruktur. |
| GreenMarble | `9` | Grön marmorstruktur. |
| MediumWood | `10` | Medium trästruktur. |
| Newsprint | `11` | Tidningspappers textur. |
| Oak | `12` | Ekstruktur. |
| PaperBag | `13` | Textur av papperspåse. |
| Papyrus | `14` | Papyrus textur. |
| Parchment | `15` | Pergamentstruktur. |
| PinkTissuePaper | `16` | Rosa silkespappersstruktur. |
| PurpleMesh | `17` | Lila mesh textur. |
| RecycledPaper | `18` | Textur av återvunnet papper. |
| Sand | `19` | Sandstruktur. |
| Stationery | `20` | Textur av brevpapper. |
| Walnut | `21` | Valnötsstruktur. |
| WaterDroplets | `22` | Vattendroppar textur. |
| WhiteMarble | `23` | Vit marmorstruktur. |
| WovenMat | `24` | Vävd matt struktur. |

### Se även

* namnutrymme [Aspose.Words.Drawing](../../aspose.words.drawing/)
* hopsättning [Aspose.Words](../../)


