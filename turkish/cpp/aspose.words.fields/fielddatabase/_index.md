---
title: "Aspose::Words::Fields::FieldDatabase class"
linktitle: "FieldDatabase"
second_title: "C++ için Aspose.Words API Referansı"
description: "Aspose::Words::Fields::FieldDatabase sınıfı. DATABASE alanını uygular. Daha fazla bilgi için C++'daki belge makalesini ziyaret edin."
type: docs
weight: 28000
url: /tr/cpp/aspose.words.fields/fielddatabase/
---
## FieldDatabase class


DATABASE alanını uygular. Daha fazla bilgi edinmek için [Working with Fields](https://docs.aspose.com/words/cpp/working-with-fields/) dokümantasyon makalesini ziyaret edin.

```cpp
class FieldDatabase : public Aspose::Words::Fields::Field,
                      public Aspose::Words::Fields::IFieldCodeTokenInfoProvider
```

## Yöntemler

| Yöntem | Açıklama |
| --- | --- |
| [FieldDatabase](./fielddatabase/)() |  |
| [get_Connection](./get_connection/)() | Veriye bir bağlantı alır. |
| [get_DisplayResult](../field/get_displayresult/)() | Görüntülenen alan sonucunu temsil eden metni alır. |
| [get_End](../field/get_end/)() const | Alan sonunu temsil eden düğümü alır. |
| [get_FieldEnd](../field/get_fieldend/)() const | Alan sonunu temsil eden düğümü alır. |
| [get_FieldStart](../field/get_fieldstart/)() const | Alan başlangıcını temsil eden düğümü alır. |
| [get_FileName](./get_filename/)() | Veritabanının tam yolunu ve dosya adını alır. |
| [get_FirstRecord](./get_firstrecord/)() | Eklenecek ilk veri kaydının tam sayı kayıt numarasını alır. |
| [get_Format](../field/get_format/)() | Alan biçimlendirmesine tipli erişim sağlayan bir [FieldFormat](../fieldformat/) nesnesi alır. |
| [get_FormatAttributes](./get_formatattributes/)() | Tabloya uygulanacak formatın hangi özniteliklerini alır. |
| [get_InsertHeadings](./get_insertheadings/)() | Sonuç tablosunda veritabanından alan adlarını sütun başlıkları olarak ekleyip eklemeyeceğini alır. |
| [get_InsertOnceOnMailMerge](./get_insertonceonmailmerge/)() | Birleştirmenin başına veri ekleyip eklemeyeceğini alır. |
| [get_IsDirty](../field/get_isdirty/)() | Alan'ın mevcut sonucunun, belgeye yapılan diğer değişiklikler nedeniyle artık doğru (eski) olup olmadığını alır veya ayarlar. |
| [get_IsLocked](../field/get_islocked/)() | Alan'ın kilitli olup olmadığını (sonucunu yeniden hesaplamamalı) alır veya ayarlar. |
| [get_LastRecord](./get_lastrecord/)() | Eklenecek son veri kaydının tam sayı kayıt numarasını alır. |
| [get_LocaleId](../field/get_localeid/)() | Alan'ın LCID'sini alır veya ayarlar. |
| [get_Query](./get_query/)() | Veritabanını sorgulayan bir SQL talimatları kümesini alır. |
| [get_Result](../field/get_result/)() | Alan ayırıcı ile alan sonu arasındaki metni alır veya ayarlar. |
| [get_Separator](../field/get_separator/)() | Alan ayırıcıyı temsil eden düğümü alır. **null** olabilir. |
| [get_Start](../field/get_start/)() const | Alan başlangıcını temsil eden düğümü alır. |
| [get_TableFormat](./get_tableformat/)() | Veritabanı sorgusunun sonucuna uygulanacak formatı alır. |
| virtual [get_Type](../field/get_type/)() const | Microsoft Word alan türünü alır. |
| [GetFieldCode](../field/getfieldcode/)() | Alan başlangıcı ile alan ayırıcı (veya ayırıcı yoksa alan sonu) arasındaki metni döndürür. Hem alan kodu hem de alt alanların alan sonucu dahil edilir. |
| [GetFieldCode](../field/getfieldcode/)(bool) | Alan başlangıcı ile alan ayırıcı (veya ayırıcı yoksa alan sonu) arasındaki metni döndürür. |
| [GetType](./gettype/)() const override |  |
| [Is](./is/)(const System::TypeInfo\&) const override |  |
| virtual [Remove](../field/remove/)() | Alanı belgeden kaldırır. Alanın hemen sonrasındaki bir düğüm döndürür. Alanın sonu, üst düğümünün son çocuğuysa, üst paragrafını döndürür. Alan zaten kaldırılmışsa, **null** döndürür. |
| [set_Connection](./set_connection/)(const System::String\&) | Veriye bir bağlantı ayarlar. |
| [set_FileName](./set_filename/)(const System::String\&) | Veritabanının tam yolunu ve dosya adını ayarlar. |
| [set_FirstRecord](./set_firstrecord/)(const System::String\&) | Eklenecek ilk veri kaydının tam sayı kayıt numarasını ayarlar. |
| [set_FormatAttributes](./set_formatattributes/)(const System::String\&) | Tabloya uygulanacak formatın hangi özniteliklerini ayarlar. |
| [set_InsertHeadings](./set_insertheadings/)(bool) | Sonuç tablosunda veritabanından alan adlarını sütun başlıkları olarak ekleyip eklemeyeceğini ayarlar. |
| [set_InsertOnceOnMailMerge](./set_insertonceonmailmerge/)(bool) | Birleştirmenin başına veri ekleyip eklemeyeceğini ayarlar. |
| [set_IsDirty](../field/set_isdirty/)(bool) | [Aspose::Words::Fields::Field::get_IsDirty](../field/get_isdirty/) için ayarlayıcı. |
| [set_IsLocked](../field/set_islocked/)(bool) | [Aspose::Words::Fields::Field::get_IsLocked](../field/get_islocked/) için ayarlayıcı. |
| [set_LastRecord](./set_lastrecord/)(const System::String\&) | Eklenecek son veri kaydının tam sayı kayıt numarasını ayarlar. |
| [set_LocaleId](../field/set_localeid/)(int32_t) | [Aspose::Words::Fields::Field::get_LocaleId](../field/get_localeid/) için ayarlayıcı. |
| [set_Query](./set_query/)(const System::String\&) | Veritabanını sorgulayan bir SQL talimatları kümesini ayarlar. |
| [set_Result](../field/set_result/)(const System::String\&) | [Aspose::Words::Fields::Field::get_Result](../field/get_result/) için ayarlayıcı. |
| [set_TableFormat](./set_tableformat/)(const System::String\&) | Veritabanı sorgusunun sonucuna uygulanacak biçimi ayarlar. |
| static [Type](./type/)() |  |
| [Unlink](../field/unlink/)() | Alan bağlantısını kaldırır. |
| [Update](../field/update/)() | Alan güncellemesini gerçekleştirir. Alan zaten güncelleniyorsa bir istisna fırlatır. |
| [Update](../field/update/)(bool) | Bir alan güncellemesi gerçekleştirir. Alan zaten güncelleniyorsa bir istisna fırlatır. |
## Ayrıca Bakınız

* Class [Field](../field/)
* Namespace [Aspose::Words::Fields](../)
* Library [Aspose.Words for C++](../../)
