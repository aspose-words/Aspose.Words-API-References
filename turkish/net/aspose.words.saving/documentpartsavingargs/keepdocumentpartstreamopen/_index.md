...
description: DocumentPartSavingArgs mülk. Aspose.Wordsün bir belge bölümünü kaydettikten sonra akışı açık tutması mı yoksa kapatması mı gerektiğini belirtir.
type: docs
weight: 40
...
Varsayılan`yanlış` ve Aspose.Words, sağladığınız akışı [`DocumentPartStream`](../documentpartstream/) içine bir belge parçası yazdıktan sonra özellik. Belirtin`doğru` akışı açık tutmak için. Çağrıda sağlanan ana çıkış akışının olduğunu lütfen unutmayın.[`Save`](../../../aspose.words/document/save/)veya [`Save`](../../../aspose.words/document/save/) olsa bile Aspose.Words tarafından asla kapatılmayacaktır.`KeepDocumentPartStreamOpen` ayarlandı`yanlış`.
