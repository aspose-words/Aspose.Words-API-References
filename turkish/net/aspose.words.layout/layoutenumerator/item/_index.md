---
title: LayoutEnumerator.Item
second_title: Aspose.Words for .NET API Referansı
description: LayoutEnumerator mülk. Varlığın adlandırılmış bir özelliğini alır.
type: docs
weight: 40
url: /tr/net/aspose.words.layout/layoutenumerator/item/
---
## LayoutEnumerator indexer

Varlığın adlandırılmış bir özelliğini alır.

```csharp
public object this[string key] { get; }
```

| Parametre | Tanım |
| --- | --- |
| key | Özelliğin adı (büyük/küçük harfe duyarlı). |

### Geri dönüş değeri

Özellik mevcut değilse boş, aksi takdirde mülkün değeri.

### Notlar

Bu şu anda aralıkların yazı tipi özelliklerini almak için kullanılıyor. Bkz.[`Font`](../../../aspose.words/font/) olası özellik adları için sınıf. Tüm özellikler desteklenmez.

### Ayrıca bakınız

* class [LayoutEnumerator](../)
* ad alanı [Aspose.Words.Layout](../../layoutenumerator/)
* toplantı [Aspose.Words](../../../)


